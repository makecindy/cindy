...
describe('parseLegalSegments(单 key 内联链接标记,词序无关)', () => {
  ...
  it('无标记文本原样单段透传', () => {
    expect(parseLegalSegments('plain')).toEqual([{ kind: 'text', text: 'plain' }]);
  });
});

describe('LEGAL_LINKS(区域分流,测试构建 = cn)', () => {
  it('cn 构建 → protocol.xd.cn 双链接', () => {
    expect(LEGAL_LINKS.termsOfService).toBe('https://protocol.xd.cn/cindy/agreement.html');
    expect(LEGAL_LINKS.privacyPolicy).toBe('https://protocol.xd.cn/cindy/privacy-1.0.html');
  });

  it('global 分支 URL 固化在源码(protocol.xd.com 双链接)', () => {
    const source = readFileSync(resolve(process.cwd(), 'src/config/legalLinks.ts'), 'utf8');
    expect(source).toContain('https://protocol.xd.com/cindy/agreement-1.0.html');
...
describe('consent 双态色板(figma wave5 SVG 源码核对值,与桌面 --login-consent-* 同源)', () => {
  ...
  it('弹窗遮罩黑 85% 两模式同值;次级钮 wave5 双色小按钮双态', () => {
    ...
    expect(loginPalettes.light.overlaySecondaryPressed).toBe('rgba(0, 0, 0, 0.1)');
    expect(loginPalettes.dark.overlaySecondaryPressed).toBe('rgba(0, 0, 0, 0.2)');
  });
});

describe('consent 文案(4 语 catalog 全给标记链接段)', () => {
  it.each(['zh-CN', 'en', 'ja', 'ko'] as const)('%s:statement 与 body 均含 terms/privacy 标记', (locale) => {
