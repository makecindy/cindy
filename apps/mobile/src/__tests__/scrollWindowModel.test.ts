...
describe('scrollWindowModel', () => {
  ...
  it('restores the load-earlier affordance after reopen even when no new content was fetched', () => {
    ...
    // token 未变化和旧 Desktop 的 metaChanged=false 分支都会补设 hasOlderMessages，
    // 用服务端总数 vs in-store 推断。
    expect(source).toMatch(
      /hasOlderMessagesAfterReopen\(\s*sessionMeta\._count\?\.messages,\s*remoteSessionStore\.getMessages\(sessionId\),\s*\)/,
    );
    ...
    expect(source).toContain('remoteSessionStore.resetMessagesFromSync(sessionId, historyPage, {');
    expect(source).toMatch(
