 ...
 */
import { hasUsableConnectedSource } from '@cindy/model-providers/registry';
...
export function agentAuthGateVerdict(input: AgentAuthGateInput): AgentAuthGateVerdict {
  ...
  return hasUsableConnectedSource(input.providers, input.agentKind, undefined, {
...
export function agentAuthGateHint(agentKind: 'claude-code' | 'codex' | 'pi' | 'grok-build'): string {
  const label =
