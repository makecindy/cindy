...
export function useMobileLocalAttachments(
  ...
  const controller = useMemo(() => createMobileLocalAttachmentUploadController({
    ...
    upload: (candidate, fileUri, opts) => uploadMobileAttachmentFromFile(candidate, fileUri, { ...opts, meetingId: candidate.meetingId }),
    ...
    controller.enqueue(
      attachmentScopeKey == null
        ...
        : candidates.map((candidate) => ({
            ...
            attachmentScopeKey,
            meetingId: parseMeetingPeer(optionsRef.current.deviceId ?? '')?.meetingId,
          })),
      opts,
    );
  };
