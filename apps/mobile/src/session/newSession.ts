...
/**
 * 需要被控端 Cindy hosted loop 才会注册的 runtime。Claude Code / Codex / Pi 的
 * 二进制随桌面端分发,几乎总是注册;grok-build 跟 Pi runtime,因此不进
...
export function normalizeNewSessionAgentKind(value: unknown): NewSessionAgentKind | null {
  return NEW_SESSION_AGENT_OPTIONS.some((option) => option.kind === value)
    ? (value as NewSessionAgentKind)
    : null;
}
