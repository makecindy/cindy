...
>(function ConversationShareSvg({ allShareableIds, colors, messages, width }, ref) {
  const svgRef = useRef<Svg | null>(null);
  const layout = useMemo(
    () => buildConversationShareSvgLayout({ allShareableIds, colors, messages, width }),
    ...
    () => ({
      exportPng() {
        if (renderSize.sourceTooLarge) {
          return Promise.reject(new Error("conversation share content is too large"));
        }
        const svg = svgRef.current;
        if (!svg) return Promise.reject(new Error("conversation share svg renderer is unavailable"));
        return new Promise<string>((resolve, reject) => {
          const timer = setTimeout(
            () => reject(new Error("conversation share svg export timed out")),
            20_000,
          );
          svg.toDataURL(
            (base64) => {
              ...
              else reject(new Error("conversation share svg export was empty"));
            },
          );
        });
      },
  ...
  return (
    <View
      ...
      pointerEvents="none"
      style={[styles.hidden, { height: renderSize.height, width: renderSize.width }]}
    >
      <Svg
      ...
      >
        <Rect fill={colors.background} height={layout.height} width={layout.width} />
        {!renderSize.sourceTooLarge ? (
          <>
            ...
            <SvgImage
              ...
              y={layout.footerY + (SHARE_CHARACTER_SIZE - SHARE_LOGO_HEIGHT) / 2}
            />
          </>
        ) : null}
      </Svg>
