/**
 * agentKindConversion —— DB/renderer 形态('cc' | 'codex' | 'pi' | 'kimi')与 maker-core
 * 形态('claude-code' | 'codex' | 'pi' | 'kimi-code')的唯一双向映射。
 *
...
export function makerToDbAgentKind(maker: string | null | undefined): DbAgentKind {
  ...
  if (maker === 'kimi-code') return 'kimi';
...
export function normalizeDbAgentKind(value: string | null | undefined): DbAgentKind {
  return value === 'codex' || value === 'pi' || value === 'kimi' ? value : 'cc';
}
