/**
 * agentKindConversion —— DB/renderer 形态('cc' | 'codex' | 'pi' | 'grok-build')与
 * maker-core 形态('claude-code' | 'codex' | 'pi' | 'grok-build')的唯一双向映射。
 *
...
export type DbAgentKind = 'cc' | 'codex' | 'pi' | 'grok-build';
/** maker-core / IPC 契约侧的 agent 形态。 */
export type MakerAgentKindWire = 'claude-code' | 'codex' | 'pi' | 'grok-build';

export function dbToMakerAgentKind(db: string | null | undefined): MakerAgentKindWire {
  if (db === 'codex') return 'codex';
  if (db === 'pi') return 'pi';
  if (db === 'grok-build') return 'grok-build';
...
export function makerToDbAgentKind(maker: string | null | undefined): DbAgentKind {
  ...
  if (maker === 'grok-build') return 'grok-build';
...
export function normalizeDbAgentKind(value: string | null | undefined): DbAgentKind {
  return value === 'codex' || value === 'pi' || value === 'grok-build' ? value : 'cc';
}
