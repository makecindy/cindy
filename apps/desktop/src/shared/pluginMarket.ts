...
export interface PluginRecoveryCandidate {
  ...
  readiness: 'ready' | 'review' | 'deferred';
  reason:
    | 'exact-match'
    | 'source-mismatch'
    | 'manifest-mismatch'
...
export interface PluginRecoveryProposal {
  ...
  counts: { ready: number; review: number; deferred: number };
  totalCount: number;
  truncated: boolean;
  /** Silences the one-time reminder only; candidates remain visible on the Plugins page. */
