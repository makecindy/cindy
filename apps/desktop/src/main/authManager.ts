  ...
  try {
    const outcome = await runGuardedRuntimeAuthExpiry({
      ...
      commit: ({
        ...
        if (opts.preservePersistedRefreshToken || isPassiveSharedUserDataInstance()) {
          return commitOwnerExpiry();
        }
        return commitRuntimeAuthExpiryInCredentialBoundary({
          validateBeforeCommit,
          withCredentialBoundary: (operation) =>
            transactAuthAccountVault(() => undefined, operation, {
              waitWhileBusyAfterRotation: true,
            }),
          commit: commitOwnerExpiry,
        }).then((commitOutcome) => {
          if (commitOutcome === 'committed') return;
          throw new AuthApiError(
            'AUTH_FLOW_SUPERSEDED',
            409,
            'Runtime auth expiry credential generation changed before commit',
          );
        });
      },
    });
    ...
    if (restoreRetainedRuntime) {
      try {
        if (!authSessionRestore) {
          log.error('runtime auth restore hook is not registered');
        } else {
