...
describe('orcaTeamStore', () => {
  ...
  it('prepares terminal cleanup before committing the team status', async () => {
    ...
    await seedOrcaWorkers(client);
    await client.exec(
      "UPDATE sessions SET list_preview = 'pending', list_preview_role = 'user' WHERE id = ?",
      ['worker-session-1'],
    );
    await client.exec(
    ...
    await expect(client.queryOne(
      'SELECT list_preview, list_preview_role FROM sessions WHERE id = ?',
      ['worker-session-1'],
    )).resolves.toEqual({ list_preview: null, list_preview_role: null });
  ...
  it('rolls the final pre-vendor sweep back when the terminal status write fails', async () => {
    ...
    await expect(client.queryOne(
      'SELECT list_preview, list_preview_role FROM sessions WHERE id = ?',
      ['worker-session-1'],
    )).resolves.toEqual({ list_preview: 'pending', list_preview_role: 'user' });
