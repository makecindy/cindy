...
describe('0090 xAI provider provenance migration', () => {
  it('namespaces only historical Pi xai sessions and schedules', () => {
    ...
    try {
      db.exec(`
        ...
        INSERT INTO sessions (id, agent_kind, provider_id) VALUES
          ('pi-legacy-custom', 'pi', 'xai'),
          ('pi-already-namespaced', 'pi', 'custom:xai'),
          ('pi-unspecified', 'pi', NULL),
          ('claude-official', 'claude-code', 'xai'),
          ('codex-official', 'codex', 'xai');
        INSERT INTO schedules (id, agent_kind, provider_id) VALUES
          ('schedule-pi-legacy-custom', 'pi', 'xai'),
          ('schedule-pi-already-namespaced', 'pi', 'custom:xai'),
          ('schedule-claude-official', 'claude-code', 'xai');
      `);
  ...
  it('is safe for partial legacy replay schemas and reruns', () => {
    ...
    try {
      db.exec(`
        ...
        VALUES ('pi-legacy-custom', 'pi', 'xai');
      `);
