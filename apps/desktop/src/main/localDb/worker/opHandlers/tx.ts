...
export function tx(db: Database.Database, args: unknown): unknown {
  ...
  switch (name) {
    ...
    case 'orca.rewindPreVendorCleanup':
      return db.transaction(() => rewindOrcaPreVendorCleanup(db, txArgs))();
...
function orcaEndTeam(
  ...
  const transaction = db.transaction(() => {
    const rewoundRows = rewindOrcaPreVendorCleanup(db, payload);
...
/** Caller holds the transaction covering both the tombstones and their list projection. */
function rewindOrcaPreVendorCleanup(
  db: Database.Database,
  args: unknown,
): Array<{ sessionId: string; clientId: string }> {
  ...
  const invalidatePreview = db.prepare(
    'UPDATE sessions SET list_preview = NULL, list_preview_role = NULL WHERE id = ?',
  );
