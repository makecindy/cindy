...
export function registerVoiceInputIpc(): void {
  ...
  ipcMain.handle('voice-input:start', async (event, payload: StartPayload | undefined): Promise<StartResult> => {
    ...
    const logger = new VoiceTimelineLogger((timelineEvent) => {
      ...
      } else if (timelineEvent.type === 'pause_refine_skipped') {
        log.info('pause refinement skipped', summarizeTimelineEventForLog(timelineEvent));
    ...
    const controller = new VoiceInputController({
      ...
      refineRequestLimit: voiceContext ? () => voiceContext.refineRequestLimit() : undefined,
