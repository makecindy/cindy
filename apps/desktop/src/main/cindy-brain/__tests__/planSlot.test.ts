...
describe('PlanSlot', () => {
  it('把合法 plan-update 投影到 Host 当前可信任务', async () => {
    ...
    expect(projector).toHaveBeenCalledWith('planner', trustedContext, {
  ...
  it('接受零任务完整快照并原样投影', async () => {
    ...
    expect(projector).toHaveBeenCalledWith('planner', trustedContext, { plan: [] });
  });
  ...
  it('多次同步每次都提交完整 Plan，后一次由同一投影入口替换前一次', async () => {
    ...
    expect(projector).toHaveBeenNthCalledWith(1, 'planner', trustedContext, {
      explanation: valid.explanation,
      plan: valid.plan,
    });
    expect(projector).toHaveBeenNthCalledWith(2, 'planner', trustedContext, {
  ...
  it('全 completed 的完整 Plan 正常投影为成功', async () => {
    ...
    expect(projector).toHaveBeenCalledWith('planner', trustedContext, { plan: completed.plan });
  });
