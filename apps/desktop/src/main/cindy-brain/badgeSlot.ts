 ...
 *
 *   - 资格审两道:声明 `notify` 卡槽 **且** `notify.badge === true`。装入时
 *     validateGhostManifest 已强制 badge 必须同时声明 panel,这里不重复判
 *     (老包不可能带 badge 字段,天然拒绝);
...
export class GhostBadgeSlot {
  ...
  handleBadge(ghostId: string, payload: unknown): GhostPipeBadgeResult {
    ...
    if (!ghost.manifest.slots?.includes('notify')) {
      return { ok: false, message: '本意识未声明 notify 卡槽,无权点亮未读角标' };
    }
