 ...
 *   打包 .cindy 到源码目录自身(id-version.cindy,同名覆盖;共享内容规则跳过
  ...
  try {
    ...
    const walk = async (
      ...
      for (const e of entries) {
        if (shouldSkipGhostPackEntry(e.name)) continue;
    ...
    // 5) 生成 zip buffer。文件收集在此之前完成 + 共享内容规则跳过 *.cindy,
