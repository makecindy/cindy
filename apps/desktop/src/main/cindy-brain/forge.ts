...
见 §4.7)、\`notify\`(弹系统轻提示,主机画壳带你的身份头;加 \`notify.badge\` 还能在插件
入口点亮持久未读绿点,见 §4.9 / §4.9.1)、\`confirm\`(弹主机
...
### 4.9.1 未读角标(notify.badge 加档)

toast 是"说一句就走",错过就没了。要留下一条**持久**的"我这儿有新内容"——
用户没去看就一直亮着——在身份卡里加一档:

\`\`\`json
{ "slots": ["panel", "notify"], "notify": { "badge": true } }
\`\`\`
...
- **必须同时声明 \`panel\`**,装入时校验,否则整个包装不进去。未读点承诺"点开
  能看到内容",没有面板的意识点亮了也无处可点,给了就是骗点击;
- 装入确认框里**单列一项**权限(不并进 \`notify\`),已装插件加这一档会触发扩权
  重新确认——常驻的注意力入口值这一次打断;
