...
const MAX_PRESET_MEDIA_BYTES = 128 * 1024 * 1024;
const IMAGE_URL_RE = /^cindy-media:\/\/blobs\/[0-9a-f]{64}\.(?:png|jpe?g|gif|webp)$/;
...
async function readPresetLibrary(): Promise<PersistedAppearancePresetLibrary> {
  try {
    ...
    return { version: 1, presets: library.presets.filter(isPreset).slice(0, MAX_PRESETS) };
  } catch {
    return { version: 1, presets: [] };
  }
}
  ...
  try {
    ...
    return isSnapshot(parsed)
      ? {
          ...
          surfaceOpacity:
            parsed.surfaceOpacity ?? GHOST_APPEARANCE_DEFAULT_SURFACE_OPACITY,
        }
      : null;
  } catch {
    return null;
...
async function assertPresetMediaBudget(
  ...
  for (const preset of library.presets) {
    if (preset.id === replacingId) continue;
  ...
  for (const hash of hashes) {
    ...
    if (total > MAX_PRESET_MEDIA_BYTES) {
      throw new Error('皮肤预设媒体总量不能超过 128 MB');
    }
  }
}
...
async function saveGhostAppearanceUnsafe(
  ...
  const persisted: PersistedAppearance = { ...snapshot };
...
export function saveGhostAppearance(
  ...
  return serializeMutation(() => saveGhostAppearanceUnsafe(snapshot, mediaHashes, ghostId, customized));
}

export async function listGhostAppearancePresets(): Promise<GhostAppearancePresetSummary[]> {
...
async function saveGhostAppearancePresetUnsafe(
  ...
  const existingIndex = library.presets.findIndex(
    (preset) => normalizePresetName(preset.snapshot.name!) === normalizedName,
  );
  const existing = existingIndex >= 0 ? library.presets[existingIndex] : undefined;
  if (!existing && library.presets.length >= MAX_PRESETS) {
    throw new Error(`最多保存 ${MAX_PRESETS} 套皮肤`);
  }
  const now = Date.now();
  ...
  await assertPresetMediaBudget(library, existing?.id, mediaHashes);
...
export async function activateGhostAppearancePreset(
  ...
  return serializeMutation(async () => {
    const library = await readPresetLibrary();
    const preset = findPreset(library.presets, idOrName);
    if (!preset) return null;
    const appearance: GhostAppearanceSnapshot = {
      ...preset.snapshot,
      dim: preset.snapshot.dim ?? GHOST_APPEARANCE_DEFAULT_DIM,
      surfaceOpacity:
        preset.snapshot.surfaceOpacity ?? GHOST_APPEARANCE_DEFAULT_SURFACE_OPACITY,
      updatedAt: Date.now(),
    };
    await saveGhostAppearanceUnsafe(
      appearance,
      ...
      { dim: preset.snapshot.dim !== undefined, surfaceOpacity: preset.snapshot.surfaceOpacity !== undefined },
    );
    return appearance;
  });
}

export function deleteGhostAppearancePreset(idOrName: string): Promise<boolean> {
  return serializeMutation(async () => {
    const library = await readPresetLibrary();
    const preset = findPreset(library.presets, idOrName);
