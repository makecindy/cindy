...
export class GhostAppearanceSlot {
  ...
  private async handleRequestUnsafe(ghostId: string, raw: unknown): Promise<GhostPipeAppearanceResult> {
    ...
    if (request.operation === 'list-presets') {
      try {
        return {
          ok: true,
          appearance: await this.deps.getCurrent(),
          presets: await this.deps.listPresets(),
        };
      } catch {
    ...
    if (request.operation === 'activate-preset') {
      ...
      try {
        const appearance = await this.deps.activatePreset(preset);
    ...
    if (request.operation === 'delete-preset') {
      ...
      try {
        if (!(await this.deps.deletePreset(preset))) {
    ...
    if (request.operation === 'save-current') {
      ...
      try {
        const preset = await this.deps.savePreset(appearance, mediaHashes, ghostId);
    ...
    if (!patching) {
      try {
        const [legacyCurrent, presets] = await Promise.all([
          ...
          this.deps.listPresets(),
        ]);
        if (legacyCurrent?.name && presets.length === 0) {
          const legacyHashes = {
    ...
    const dim = finiteInRange(
      request.dim,
      current?.dim ?? GHOST_APPEARANCE_DEFAULT_DIM,
      0,
      0.85,
    );
