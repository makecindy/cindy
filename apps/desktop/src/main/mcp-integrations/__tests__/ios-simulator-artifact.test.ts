...
describe('packaged iOS Simulator sidecar artifact verification', () => {
  ...
  it('rejects a symlinked executable before invoking codesign', async () => {
    const fixture = await createFixture();
    const executableDirectory = path.dirname(fixture.executablePath);
    const replacementDirectory = path.join(fixture.root, 'replacement');
    await rename(executableDirectory, replacementDirectory);
    await symlink(replacementDirectory, executableDirectory, 'junction');
    const commandRunner = createCommandRunner(fixture);
