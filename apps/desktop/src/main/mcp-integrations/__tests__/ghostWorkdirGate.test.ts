...
describe('Full Access 插件文件交接', () => {
  ...
  it('确认等待期间账号切换:附件授权不落进新账号的账本,直接 fail closed', async () => {
    ...
    activeOwnerScopeKeyMock
      ...
      .mockReturnValueOnce('local:owner-a:0') // 普通附件落账前的 owner 复判
  ...
  it('普通调用在插件 setup 等待期间账号切换:不把附件授权写进新账号', async () => {
    const file = path.join(outsideDir, 'normal-owner-switch-during-setup.png');
    fs.writeFileSync(file, Buffer.from('normal-owner-switch-during-setup'));
    activeOwnerScopeKeyMock
      .mockReturnValueOnce('local:owner-a:0') // setup 等待前捕获的 owner 快照
      .mockReturnValue('local:owner-b:0'); // setup 返回后的附件落账前复判
    ensureReadyMock.mockImplementationOnce(async () => {
      // 模拟 OAuth/设置确认等待期间发生 account-switch。
      activeOwnerScopeKeyMock.mockReturnValue('local:owner-b:0');
      return {
    ...
    const result = await makeDeps('codex', 'normal-owner-switch-during-setup').callGhostTool({
      ghostId: 'art',
      tool: 'run',
      args: {},
      attachments: [file],
