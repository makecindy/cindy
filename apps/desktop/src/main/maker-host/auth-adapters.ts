...
export class DesktopCodexAuthAdapter implements AuthAdapter {
  ...
  private async finishSuccessfulCodexLogin(
    ...
    // Bind before the final OAuth state read because getAccessToken() enforces
    // the owner binding gate. Otherwise an otherwise valid explicit login is
    // reported as unauthenticated on its first post-login read.
