...
export function createClaudeAutoClassifierFailureObserver(
  ...
  return (ctx: ResponseObserverCtx) => {
    ...
    // 识别成功且已升级到**会话级**的那一刻记一条,带完整计数 —— coordinator 侧的日志只覆盖
    // 「切换成功/失败」,看不到「识别到了但被 episode 阈值挡住」的那些。
    // info 级:这一刻本就低频(真的持续故障才走到),且打包版默认 level=info 会落盘。
    //
    // 阈值以下的 turn 级试探性降级刻意**不在这里打点**:它每次故障都发(见上),info 级会
    // 刷屏;那一层的可观测性由 coordinator 的 switchedTentative / skippedAlreadyFallback
    // 承担(claude-auto-permission-fallback 的 FallbackCounters)。
