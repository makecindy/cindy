...
describe('searchSessionsFn', () => {
  ...
  it('CJK 短词收成相邻 phrase，多词 AND', async () => {
    ...
    expect(dbMocks.query.mock.calls[0][1][0]).toBe('"登 录" AND "报 错"');
  });

  it('英文多词保持 AND，不退化成 OR', async () => {
    await searchSessionsFn('login bug');
    expect(dbMocks.query.mock.calls[0][1][0]).toBe('"login" AND "bug"');
  });
  ...
  it('snippet 从原文重建并透传过滤参数（不用索引侧文本）', async () => {
    ...
    expect(sql).toContain('m.content AS content');
    ...
    expect(params).toEqual(['"登 录"', 's1', 'user', 3]);
  });
