  ...
  if (!_maker) {
    ...
    const codexAgent = new CodexAgent({
      ...
      onCodexThreadUnarchived: async ({
        ...
        const resumed = await clearWorkerIdleReleaseMarker({
          workerId,
          sessionId,
          expectedIdleSince,
          expectedUpdatedAt,
        ...
        if (!resumed) {
          const error = new Error(
            `SESSION_RUNNING: worker session ${sessionId} runtime release changed during unarchive`,
          );
