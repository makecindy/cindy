  ...
  return new Promise((resolve, reject) => {
    ...
    const armTimeout = (durationMs: number, stage: string): void => {
      ...
      timer = setTimeout(() => {
        finish(() =>
          reject(
            new Error(`timed out ${stage} Windows packaged-instance barrier`),
          ),
        );
        child.kill();
      }, durationMs);
    ...
    const onStdout = (chunk: Buffer | string): void => {
      ...
      while (newline >= 0) {
        ...
        if (status.status === 'started') {
          // timeoutMs measures mutex contention only; PowerShell cold start is a separate phase.
          armTimeout(timeoutMs + 1_000, 'waiting for');
        } else if (status.status === 'locked') {
          // Add-Type + the message-window probe happen only after the mutex is safely held.
...
function waitForExit(
  ...
  return new Promise((resolve) => {
    ...
    const terminateTimer = setTimeout(() => {
      child.kill();
      // TerminateProcess is asynchronous on Windows. Keep waiting for the real exit event so a
      // retry cannot race the still-owned mutex; retain a second bounded guard for broken hosts.
