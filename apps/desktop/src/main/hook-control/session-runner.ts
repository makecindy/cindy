...
export function createMakerHookSessionRunner(deps: {
  ...
  return {
    ...
    async run(req) {
      ...
      // ignore create options. Apply the safe mode to that live instance for
      // this turn only; every terminal path below restores its original mode.
      if (reusedPermissionModeToRestore !== null) {
        try {
          await session.setPermissionMode(permissionMode);
        } catch (err) {
          await session.setPermissionMode(reusedPermissionModeToRestore).catch(() => undefined);
          return fail(err instanceof Error ? err.message : String(err));
        }
      }
