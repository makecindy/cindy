...
export function buildHookPromptNote(im: string | undefined): string {
  ...
  if (im === 'x') {
    return (
      ...
      // 这里陈述的是**机制**(session-runner 对 X 取 observer.finalSegment()),
      // 不是对模型的行为要求。先前写成"不要写过程叙述"是纯软约束, 模型不听就
      // 穿透到公开时间线(2026-08-01 实踩); 告知机制则顺应它边做边说的天性,
      // 只要求把结论收进最后一条。改这段话时同步 finalTextForChannel 的判据。
      ...
      // 未开通长推的账号单帖上限 280 字符(CJK 按 2 计),超出会被服务端
      // 截断并附「完整结果见桌面端」尾注。给出具体预算, agent 才能自己收着写;
      // 只说「简短」它无从判断该多短。
      '篇幅按一条推文控制: 目标 280 字符以内(中日韩字符按 2 个计), ' +
      '超出会被截断, 只留尾注指向桌面端。用短段落和简单列表; 不要依赖表格、' +
