...
function workflowPrerequisiteSkillPolicy(
  skillFile: string,
...
describe('prepareCodexGlobalPluginsBridge', () => {
  it('writes a durable prerequisite block into the isolated downstream Skill', async () => {
    ...
    await fs.mkdir(path.dirname(prerequisiteFile), { recursive: true });
    const prerequisiteSkill = [
      '---',
      'name: git-workflow',
      'description: Cindy publish gates',
      '---',
      '',
      '# Git workflow',
      '',
      'Register every ready PR with the repository heartbeat.',
      '',
    ].join('\n');
    await fs.writeFile(prerequisiteFile, prerequisiteSkill, 'utf8');
    ...
    const sourceSkill = '---\nname: yeet\ndescription: Publish\n---\n\n# Publish\n';
    await fs.writeFile(path.join(sourceSkillDir, 'SKILL.md'), sourceSkill, 'utf8');
    await fs.writeFile(
      paths.sourceConfigFile,
      `[plugins."${plugin}@${marketplace}"]\nenabled = true\n`,
      'utf8',
    );
    const capabilityRouting = workflowPrerequisiteSkillPolicy(prerequisiteFile);

    const first = await prepareCodexGlobalPluginsBridge(codexHome, {
      homeDir,
      capabilityRouting,
    });
    expect(first.routingFailures).toEqual([]);

    const isolatedSkillFile = path.join(
      paths.cacheDir,
      marketplace,
      plugin,
      version,
    ...
    const isolatedSkill = await fs.readFile(isolatedSkillFile, 'utf8');
    expect(isolatedSkill).toContain('<!-- cindy:skill-prerequisites:start -->');
    expect(isolatedSkill).toContain('Cindy prerequisite: `git-workflow`');
    expect(isolatedSkill).toContain(prerequisiteFile);
    expect(isolatedSkill).toContain(
      'Register every ready PR with the repository heartbeat.',
    );
    expect(isolatedSkill.indexOf('# Git workflow')).toBeLessThan(
      isolatedSkill.indexOf('# Publish'),
    );
    expect(isolatedSkill).not.toContain('description: Cindy publish gates');
    await expect(
      fs.readFile(path.join(sourceSkillDir, 'SKILL.md'), 'utf8'),
    ).resolves.toBe(sourceSkill);

    await fs.writeFile(isolatedSkillFile, sourceSkill, 'utf8');
    ...
    expect(await fs.readFile(isolatedSkillFile, 'utf8')).toContain(
      '<!-- cindy:skill-prerequisites:start -->',
    );

    await fs.writeFile(
      prerequisiteFile,
      prerequisiteSkill.replace(
        'Register every ready PR with the repository heartbeat.',
        'Check reviewDecision before declaring a ready PR complete.',
      ),
      'utf8',
    );
    const refreshed = await prepareCodexGlobalPluginsBridge(codexHome, {
      homeDir,
      capabilityRouting,
    });
  ...
  it('preserves BOM and CRLF frontmatter while binding prerequisite content', async () => {
    const { homeDir, codexHome, paths } = await setup();
    const marketplace = 'openai-curated';
    const plugin = 'github';
    const version = '0.1.0';
    const prerequisiteFile = path.join(
      homeDir,
    ...
    await fs.mkdir(path.dirname(sourceSkillFile), { recursive: true });
    const sourceSkill = '\uFEFF---\r\nname: yeet\r\n---\r\n\r\n# Publish\r\n';
    await fs.writeFile(sourceSkillFile, sourceSkill, 'utf8');
    ...
    const result = await prepareCodexGlobalPluginsBridge(codexHome, {
      homeDir,
      capabilityRouting: workflowPrerequisiteSkillPolicy(
        prerequisiteFile,
        marketplace,
      ),
    });
    ...
    const isolated = await fs.readFile(
      path.join(
        paths.cacheDir,
        marketplace,
        plugin,
    ...
    expect(isolated).toContain('# Cindy gate\r\n');
    expect(isolated.indexOf('# Cindy gate')).toBeLessThan(
      isolated.indexOf('# Publish'),
    );
    expect(isolated.replace(/\r\n/g, '')).not.toContain('\n');
    await expect(fs.readFile(sourceSkillFile, 'utf8')).resolves.toBe(sourceSkill);
  });
