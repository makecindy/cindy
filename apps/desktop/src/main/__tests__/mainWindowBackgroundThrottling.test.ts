...
describe('窗口可见性广播（装饰动画闸门的兜底信号）', () => {
  ...
  const BROADCAST_EXEMPT = new Map<string, string>([
    ...
    [
      'screen-capture/overlayCapture.ts',
      // 区域截图覆盖层是一次性选区窗口：静态冻结帧 + 用户拖拽选框，无任何常驻
      // 装饰动画；选区结束即 destroy，不存在"隐藏时动画空转"的场景。关闭节流
      // 是为了 show() 前的隐藏加载阶段不被 Chromium 降速。若将来给覆盖层加
      // 常驻动画，删掉本豁免改装广播。
      '截图选区覆盖层为一次性窗口，无常驻装饰动画',
    ],
  ]);
