...
describe('runQuitDisposers', () => {
  ...
  it.each([false, true])('keeps real bootstrap Codex and DB dependencies until Maker settles or times out (timeout=%s)', async (timesOut) => {
    ...
    const names = new Set(['shutdown-maker', 'lsp-pool', 'pi-subagent-final-sweep', 'codex-env', 'codex-proxy', 'remote-ssh-pool', 'im-device-link-db-client', 'local-db-close']);
    ...
    expect(events).toEqual(['lsp-pool', 'im-device-link-db-client']);
    if (timesOut) await vi.advanceTimersByTimeAsync(100);
    else finish();
    await quit;
    expect(events).toEqual([
      'lsp-pool', 'im-device-link-db-client', ...(timesOut ? [] : ['shutdown-maker']), 'pi-subagent-final-sweep',
      'codex-env', 'codex-proxy', 'remote-ssh-pool', 'local-db-close',
    ]);
    finish();
  });
