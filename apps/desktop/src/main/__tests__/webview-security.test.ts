...
describe('applyGhostWebviewHardening(意识面板 webview)', () => {
  it('只把可唯一识别的 panel.html 当作 Agent bridge 入口', () => {
    ...
    expect(
      isGhostPanelAgentEntry('cindy-ghost://art/ui/shared.html', 'ui/shared.html', 'ui/shared.html'),
    ).toBe(false);
  ...
  it('把 Renderer claim 覆盖为 Main 核准的 owner partition', () => {
    ...
    expect(authorizeGhostWebviewAttach(webPreferences, params, resolver)).toMatchObject({
      id: 'same-ghost',
      owner: { mode: 'cloud', dataOwnerId: 'owner-a' },
    });
