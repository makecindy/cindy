...
describe('messageHandler !stop routing', () => {
  ...
  it('calls adapter.onSlashCommandEvent before slash dispatch; hook throw does not block the command', async () => {
    const hook = vi.fn<(event: IMMessageEvent) => void>();
    // 重建一份带钩子的 adapter — 钩子先于 handleSlashCommand 执行(飞书靠它
    // 记住开话题 slash 的群主流取数 lane)。
    const im = {
      onMessage(handler: (event: IMMessageEvent) => void) {
        deliver = handler;
        return () => undefined;
    ...
    const slashEvent = makeEvent({ text: '/ctr' });
    const order: string[] = [];
    hook.mockImplementation(() => {
      order.push('hook');
    });
    handleSlashCommand.mockImplementation(async () => {
      order.push('slash');
      return true;
    });
    deliver(slashEvent);
    await flushMicrotasks();
    expect(order).toEqual(['hook', 'slash']);
    expect(hook).toHaveBeenCalledWith(slashEvent);

    // 钩子抛错只记 warn, 命令照常派发。
    hook.mockImplementation(() => {
      throw new Error('hook exploded');
    });
    const second = makeEvent({ text: '/model', messageId: 'msg-2' });
    deliver(second);
    await flushMicrotasks();
    expect(handleSlashCommand).toHaveBeenCalledWith('/model', {
      botContextId: 'bot-ctx',
      userId: 'U123456789',
    });
  });
