...
describe('交互已失效时的卡片收口', () => {
  // Telegram 的 callback token 只活在进程内存里, 重启后按钮还在但点了什么都不会发生。
  // 配了 interactionExpiredNotice 的渠道要把卡片改写成过期态(空按钮), 让按钮消失。
  const expiringAdapter = {
    ...adapter,
    channel: 'telegram',
    threadScoped: false,
  ...
  async function press(
    im: ChannelIM,
    buttonId: string,
    testAdapter: ImChannelAdapter,
    payload: Record<string, unknown> = { requestId: 'req-gone' },
  ): Promise<void> {
    const attach = createCardActionHandler(testAdapter, cards, turnRunner);
    let handler: ((e: IMCardActionEvent) => Promise<void>) | null = null;
    (im.onCardAction as ReturnType<typeof vi.fn>).mockImplementation((cb) => {
  ...
  it('pending interaction 已不在 → 卡片改写为过期态且不再带按钮', async () => {
    const im = makeIm();
    (resolvePending as ReturnType<typeof vi.fn>).mockReturnValue(false);

    await press(im, 'plan:reject', expiringAdapter);
  ...
  it('按钮 id 认不出来时同样收口(旧版本卡片留在会话里)', async () => {
    const im = makeIm();

    await press(im, 'legacy:whatever', expiringAdapter);
  ...
  it('未配置过期文案的渠道保持原行为, 不动卡片', async () => {
    const im = makeIm();
    (resolvePending as ReturnType<typeof vi.fn>).mockReturnValue(false);

    await press(im, 'plan:reject', adapter);

    expect(im.updateInteractiveCard).not.toHaveBeenCalled();
  });

  it('收口失败不外抛(卡片清理是尽力而为)', async () => {
    const im = makeIm();
    im.updateInteractiveCard.mockRejectedValueOnce(new Error('message to edit not found'));
    (resolvePending as ReturnType<typeof vi.fn>).mockReturnValue(false);

    await expect(press(im, 'plan:reject', expiringAdapter)).resolves.toBeUndefined();
    expect(mocks.logger.warn).toHaveBeenCalledWith(
      expect.stringContaining('expired interaction card cleanup failed'),
    );
  });
});
