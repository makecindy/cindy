...
私聊直接发消息就行；群里 @ 我或回复我的消息才会触发，我也会记住群里最近的聊天做上下文~`,
        ...
        missing === 'gateway-key'
          ...
          : missing === 'provider-key'
            ...
            : missing === 'provider-disconnected'
              ? '未连接或连接已失效'
              : `需要先登录 ${agentKind} 凭证`;
      const message = `⚠️ 当前 Telegram 会话使用供应商「${provider}」（${model}），${reason}。`;
      return attached
  ...
  cards: {
    ...
    control: {
      ...
      sessionPickerTitle: (displayName: string) => `🎮 ${displayName} 里的会话`,
      sessionPickerHint: '挑个会话继续 · ➕ 新建 开新会话 · ↩️ 后退 换工作区 · 🚪 退出 取消',
      ...
      takeoverLoadingPrompts: [
        (sessionTitle: string) =>
          `⏳ 正在让你接管 **${sessionTitle}**…\n_📦 加载会话元数据_  ·  _🔌 接通 agent 通道_  ·  _🧠 复盘上一轮进展_\n_三两秒就好_`,
        (sessionTitle: string) =>
          `🎮 接入 **${sessionTitle}** 中…\n_正在把上下文搬过来，马上就好_`,
        (sessionTitle: string) =>
      ...
      newSessionWelcomePrompts: [
        ...
        (workspaceName: string) =>
          `✨ 新会话已建——工作目录 **${workspaceName}** 已经备好，agent 等你发话。\n第一条消息开聊；\`/exctr\` 随时退出`,
        (workspaceName: string) =>
          `🎮 准备就绪——在 **${workspaceName}** 给你开了个全新会话。\n要做啥告诉我，agent 在线~  \`/exctr\` 收手`,
        (workspaceName: string) =>
