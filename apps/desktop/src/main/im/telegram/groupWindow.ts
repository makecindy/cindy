...
/**
 * 存储不做 TTL / 条数自动清理(Chris 2026-07-30 拍板): Telegram bot 没有别的聊天
 * 记录来源, 本地群消息库就是它的记忆, 清理只在用户明确要求时执行(与主流 agent
 * 产品同理念)。单条正文截断与每轮 4000 字注入预算仍在, 那是 prompt 预算, 不是
 * 存储上限。
 *
 * 上面这段原本写的是"永久保留, 通过不向共享核心传 retention 表达" —— 已不成立:
 * 下面确实传了 `TELEGRAM_PERSONAL_GROUP_WINDOW_RETENTION`。Chris 2026-08-08 定的是
 * **按容量兜底、不按条数**(每命名空间 1 GiB 正文 + 500 万行安全阀), 日常量级碰不到,
 * 它是安全阀不是日常清理 —— 与"不按条数挤掉去年的内容"是同一个意思, 不是推翻。
 */
