...
export function buildLinuxUpdateScript(params: LinuxUpdateScriptParams): string {
  ...
  return [
    ...
    '    exit 1',
    'fi',
    `INSTALL_DEB=${qScript}.deb`,
    `cp -f ${qDeb} "$INSTALL_DEB"`,
    'chmod 600 "$INSTALL_DEB"',
    `ACTUAL_SHA=$(sha256sum "$INSTALL_DEB" | awk '{print $1}')`,
    ...
    '    exit 1',
    'fi',
    `echo "[$(date)] private .deb copy sha256 revalidated" >> ${qLog}`,
    '',
    'INSTALL_EXIT=1',
    'if [ -x /usr/bin/apt-get ]; then',
    '    echo "[$(date)] installing via apt-get: $INSTALL_DEB" >> ${qLog}',
    '    "$PKEXEC" /usr/bin/apt-get install --yes --allow-downgrades "$INSTALL_DEB" >> ${qLog} 2>&1',
    '    INSTALL_EXIT=$?',
    'elif [ -x /usr/bin/dpkg ]; then',
    '    echo "[$(date)] installing via dpkg: $INSTALL_DEB" >> ${qLog}',
    '    "$PKEXEC" /usr/bin/dpkg --install "$INSTALL_DEB" >> ${qLog} 2>&1',
    '    INSTALL_EXIT=$?',
    'else',
