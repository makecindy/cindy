...
describe('shared task context usage cold-start authority', () => {
  it.each(['ask', 'bypassPermissions'])('uses only the host snapshot, preserving host mode %s', async (permissionMode) => {
    const guard = createSessionMeetingContextUsageGuard(capture, 'task');
  ...
  it('rejects cross-task and revoked requests before reading the task', async () => {
    ...
    for (const guard of [
      createSessionMeetingContextUsageGuard(capture, 'other-task'),
      createSessionMeetingContextUsageGuard({ ...capture, isCurrent: () => false }, 'task'),
      createSessionMeetingContextUsageGuard({ ...capture, authorize: () => false }, 'task'),
  ...
  it('rejects revocation during the DB read and again before a delayed bootstrap side effect', async () => {
    let current = true;
    const guard = createSessionMeetingContextUsageGuard({ ...capture, isCurrent: () => current }, 'task');
  ...
  it('preserves ordinary local and same-account remote options without a DB lookup', async () => {
    const guard = createSessionMeetingContextUsageGuard(undefined, 'task');
    const read = vi.fn(async () => ({}));
