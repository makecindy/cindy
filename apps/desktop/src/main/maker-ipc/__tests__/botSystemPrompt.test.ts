...
describe('稳定层:能力必须写进提示词', () => {
  ...
  it('记忆 / 技能 / 协作各自按信号出现', () => {
    ...
    expect(all).toContain('清晰稳定的偏好、纠正与长期背景第一次出现就记');
    expect(all).toContain('不要在普通一轮里默认记流水账或临时状态');
    expect(all).toContain('拿不准是否长期有效时只问一句');
    expect(all).not.toContain('同一偏好、纠正或长期背景稳定重复出现');
    expect(all).not.toContain('不要等他重复第二次');
    expect(all).not.toContain('第一次明确说出');
    expect(all).not.toContain('不要把一次性表达当成长期记忆');
