...
/** New Maker 草稿面 wire 入口:全部可选 harness(含 Grok Build)都有草稿槽。 */
...
describe('agentKind IPC gate', () => {
  ...
  it('accepts every selectable harness including Grok Build at the draft gate', () => {
    expect([...DRAFT_AGENT_KINDS]).toEqual(['claude-code', 'codex', 'pi', 'grok-build']);
    for (const kind of DRAFT_AGENT_KINDS) {
      expect(requireDraftAgentKind(kind)).toBe(kind);
    }
    expect(() => requireDraftAgentKind('grok')).toThrow('[INVALID_PARAMS]');
    // 草稿 pref 的字段叫 agent，报错要指回调用方的参数名。
    expect(() => requireDraftAgentKind('grok', 'agent')).toThrow('invalid agent: grok');
  });
