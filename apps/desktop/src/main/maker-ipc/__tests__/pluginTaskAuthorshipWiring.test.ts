...
it('marks plugin dispatch and every queue fallback with the host-only receipt', () => {
  ...
  expect(dispatch).toContain("message: text, autoReviewUserText: '', forceQueue: true");
...
it('keeps empty receipts distinct from missing authorship on direct persistence', () => {
  ...
  expect(build({ autoReviewUserText: '' }, undefined)).toEqual({ autoReviewUserText: '', delivery: 'turn' });
  expect(build({}, undefined)).toBeUndefined();
  expect(build({ autoReviewUserText: '' }, { kind: 'orca' })).toMatchObject({ origin: { kind: 'orca' }, autoReviewUserText: '' });
});
...
it('builds a durable queued plugin input without promoting plugin text to user intent', async () => {
  ...
  const queued = JSON.parse(JSON.stringify(await build({ ...base, autoReviewUserText: '' })));
  expect(queued).toMatchObject({ text: base.message, persistedContent: base.persistedContent, autoReviewUserText: '', permissionMode: 'auto' });
