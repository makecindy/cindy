...
export interface AgentInputCoordinatorDeps {
  ...
  rewindPersistedUserMessageAfterClear?: (sessionId: string, clientId: string,
...
export class AgentInputCoordinator {
  ...
  private async restoreQueueSnapshot(sessionId: string): Promise<void> {
    ...
    // 排队/直发,不丢任务只丢陈旧副本。
    const restorable = boundaryFilteredItems.filter((item) => !isActiveTurnCleanupSnapshotItem(item) && !existingIds.has(item.clientId),
    );
  ...
  shouldQueueNewTurn(sessionId: string): boolean {
    ...
    return (
      ...
      state.queueEditLocks.length > 0 || state.recovery !== null ||
  ...
  hasQueuedItemWhere(
    ...
    return state.activeTurnCleanupRecoveryItem
      ? predicate(state.activeTurnCleanupRecoveryItem) : false;
  }
  ...
  private async drain(sessionId: string, reason: string): Promise<void> {
    ...
    try {
      ...
      try {
        result = await this.deps.sendToAgent(sessionId, makerUserMessage, head.createOpts, {
          onDispatching: () => {
            if (this.isActiveTurnCurrent(sessionId, active)) {
              active.dispatchLifecycle = 'sending';
            }
