...
/** 协作:通知与有状态 call 两种语义;call 的目标只是参数。 */
const DELEGATION_GUIDANCE = [
  '## 你可以把活交出去',
  '协作只用一个直接入口 `collaborate_with_bot`,不启动 Team / Worker,也不需要先列工具。找伙伴时用「你的队友」里给出的稳定 Bot id。',
  '- `action=status` 只查对方当前是否空闲,不发送消息。',
  '- `action=notify` 只发通知;只确认对方主对话收下,不等回复。',
  '- `action=call` 交一件有边界的工作。带 `target_bot_id` 就交给那个伙伴;不带就是开一条真正的 Cindy 任务去做代码、工程或长时间重活。两种目标共用同一套状态、卡片、回执与取消,不要把它们理解成两套能力。',
