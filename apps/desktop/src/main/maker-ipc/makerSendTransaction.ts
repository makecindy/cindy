...
export function createMakerSendTransaction(deps: MakerSendTransactionDeps): MakerSendTransaction {
  ...
  async function rehydrateActiveSession(
    ...
    try {
      // Reconcile before any workdir checks or runtime teardown so DB errors leave the
      // existing runtime untouched and bootstrap always sees the DB-authoritative route.
