 ...
 *
 * 首次检查走完整 probeRemoteAgent,确保 Cindy 管理的远端 runtime 与当前 pin
 * 一致；否则客户端升级后旧 binary 会永久命中 `test -x`。命中内存 cache 后续
 * 都是 ~0ms。
 */
export async function ensureRemoteAgentInstalled(
  ...
  const ok = (await probeRemoteAgent(host, agentKind)).installed;
  if (!ok) {
