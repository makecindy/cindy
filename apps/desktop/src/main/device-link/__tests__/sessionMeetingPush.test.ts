...
import type { SessionMeetingHost } from '../sessionMeetingHost';

const guestA = 'meeting~meeting-a~guest~member-a~device-a';
const guestB = 'meeting~meeting-b~guest~member-b~device-b';
...
beforeEach(() => {
  ...
  setSessionMeetingDispatchHost({ capturePeer(source: string) {
    const sessionId = grants.get(source);
    if (!sessionId) return null;
    const current = () => grants.get(source) === sessionId;
    return {
      author: { meetingId: 'meeting-a', sessionId, memberId: 'member-a', accountId: 'guest', displayName: 'Guest' },
      isCurrent: current, authorize: current,
    };
  } } as unknown as SessionMeetingHost);
});
afterEach(() => {
  __testing.reset();
