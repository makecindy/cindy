...
import { assertSessionMeetingReferences } from '../sessionMeetingDispatch.js';
...
describe('rewriteOutboundMedia — channel gating', () => {
  it('uploads a real Desktop composer image even when its payload says desktop-host', async () => {
    ...
    uploadLocalFile.mockResolvedValue({ key: 'cindy/shared-task/meeting/u/a.png', contentType: 'image/png', size: 10, sha256: SHA256 });
    ...
    const result = await withSessionMeetingMedia('meeting', () => rewriteOutboundMedia('maker:input:enqueue', ['task', item]));
    expect(uploadLocalFile).toHaveBeenCalledWith('/cache/a.png', { contentType: 'image/png' });
    expect(() => assertSessionMeetingReferences(result[1], 'task', 0, 'meeting')).not.toThrow();
  });
  it('rewrites newly added shared queue-edit attachments and preserves host-owned existing files', async () => {
    uploadLocalFile.mockResolvedValue({ key: 'cindy/shared-task/meeting/u/new.png', contentType: 'image/png', size: 10, sha256: SHA256 });
    ...
    const result = await withSessionMeetingMedia('meeting', () => rewriteOutboundMedia('maker:input:update-content', ['task', 'client', item], new Set(['/host/cache/old.png'])));
    ...
    expect(parseAttachmentOssRef(rewritten.files[1].path)?.ossKey).toBe('cindy/shared-task/meeting/u/new.png');
...
describe('rewriteQueued — persistedContent 同批改写 + 去重单上传', () => {
  it('sends a shared Desktop attachment without leaking local optimistic/retry paths', async () => {
    ...
    const result = await withSessionMeetingMedia('shared', () => rewriteOutboundMedia('maker:input:enqueue', ['task', input]));
    expect(() => assertSessionMeetingReferences(result[1], 'task', 0, 'shared')).not.toThrow();
    expect(uploadLocalFile).toHaveBeenCalledTimes(1);
