...
let batchLookup: BatchLookup | null = null;

export function setRemoteBotSessionLookup(
  value: Lookup | null,
  batch: BatchLookup | null = null,
): void {
  lookup = value;
...
function sessionIds(value: unknown): string[] {
  ...
  return [
    row.sessionId,
    row.parentSessionId,
    record(row.session)?.id,
    record(row.message)?.sessionId,
  ].filter((id): id is string => typeof id === 'string' && id.length > 0);
}

/** Resolve channel-specific Bot IDs before checking generic Session references. */
export async function assertRemoteBotInvocationAllowed(
  args: unknown[],
  channel = '',
): Promise<void> {
  if (!lookup) return;
  ...
  const ids = new Set(
    args.flatMap((arg, index) => [
      ...(index === 0 && typeof arg === 'string' ? [arg] : []),
      ...sessionIds(arg),
    ]),
  );
  for (const id of ids) {
    if ((await lookup(id, channel.startsWith('local-db:bots:') ? 'bot' : 'session')) === 'hidden')
  ...
  for (const arg of args) {
    ...
    if (
      ...
      (await lookup(ref.id, 'bot')) === 'hidden'
    ) {
      throw new Error('[NOT_FOUND] Resource does not exist');
    }
  }
...
export async function projectRemoteSessionResult(
  channel: string,
  value: unknown,
): Promise<unknown> {
  if (
  ...
  const reference = (item: unknown) => {
    ...
    return {
      id,
      kind:
        ref?.kind === 'bot' || channel.startsWith('local-db:bots:')
          ? ('bot' as const)
          : ('session' as const),
    };
  };
  const project = async (item: unknown) => {
    ...
    if (!ref) return item;
    const access = await lookup!(ref.id, ref.kind);
    return access === 'hidden' ? null : item;
  };
  if (Array.isArray(value)) {
    const batch = batchLookup;
    if (!batch) {
      const projected: unknown[] = [];
      for (const item of value) {
        const result = await project(item);
        if (result !== null) projected.push(result);
      }
      return projected;
    }
    const refs = value.map(reference);
    const access = new Map<string, ReadonlyMap<string, RemoteBotSessionAccess>>();
    // Batch only within this authorization pass. Replays must re-read visibility;
    // sharing a cached verdict across requests could disclose a newly hidden Bot.
    for (const kind of ['session', 'bot'] as const) {
      const ids = [...new Set(refs.filter((ref) => ref?.kind === kind).map((ref) => ref!.id))];
      if (ids.length) access.set(kind, await batch(ids, kind));
    }
    return value.filter((item, index) => {
      const ref = refs[index];
      return (
        item !== null && (!ref || (access.get(ref.kind)?.get(ref.id) ?? 'hidden') !== 'hidden')
      );
    });
  }
  const row = record(value);
  if (row && !row.id && Array.isArray(row.sessions))
    return { ...row, sessions: await projectRemoteSessionResult(channel, row.sessions) };
  if (row && Array.isArray(row.items)) {
    ...
    return {
      ...
      ...(items.length !== row.items.length
        ? { revision: items.map((item) => record(item)?.revision ?? '').join('|') }
        : {}),
    };
  }
  const projected = await project(value);
  if (projected === null && channel.endsWith(':get'))
    throw new Error('[NOT_FOUND] Resource does not exist');
  return projected;
}
...
export function hasRemoteBotSessionLookup(): boolean {
  return lookup !== null;
}
