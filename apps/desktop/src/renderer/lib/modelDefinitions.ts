...
function toLegacy(m: ModelDescriptor, vendorKey: 'cc' | 'codex' | 'pi' | 'kimi'): ModelDefinition {
  return {
    id: m.id,
    label: m.displayName,
...
function defaultMarkerAgentForVendor(
  ...
  if (vendorKey === 'kimi') return 'kimi-code';
  return vendorKey;
}
...
export function coldStartModelIdForVendor(vendorKey: 'cc' | 'codex' | 'pi' | 'kimi'): string {
...
function coldStartLabelForVendor(vendorKey: 'cc' | 'codex' | 'pi' | 'kimi'): string {
