...
/** Only call with a current authenticated meeting list/detail response, before linking. */
export function bindSharedTaskPushOwner(peer: string, hostAccountId: string): void {
  if (parseMeetingPeer(peer)?.role !== 'host' || !hostAccountId) return;
...
export function isRemoteDataOwnerPushCurrent(
  ...
  const meetingPeer = parseMeetingPeer(remoteDeviceId);
  const sharedOwner = meetingPeer ? sharedTaskOwners.get(remoteDeviceId) : undefined;
  if (meetingPeer && (meetingPeer.role !== 'host' || !sharedOwner ||
  ...
  if (meetingPeer && sourceEpoch !== undefined) {
  ...
  } else if (meetingPeer && sharedHostSourceEpochs.has(remoteDeviceId)) return false;
