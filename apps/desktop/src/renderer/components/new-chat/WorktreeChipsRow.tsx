...
export function WorktreeChipsRow({
  ...
  const baseRepo =
    ...
    !detect.data.isInsideWorktree
      ...
      : null;
  // 「确认不合格」只认**成功回包**里的三种资格缺失;探测中(data 未到)与探测失败
  // (data 为 null 且不 loading)都是 null——「确认不是 git」和「探测不出来」不是
  // 一回事,后者必须维持 fail closed,不能被当成放行依据。
  const confirmedIneligible: boolean | null = detect.data
