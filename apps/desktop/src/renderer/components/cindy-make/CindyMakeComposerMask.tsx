...
/** Same footprint as TakeoverMask; interaction prompts are rendered ahead of this mask. */
export function CindyMakeComposerMask({
  ...
  const heading =
    phase === 'executing'
      ? 'cindyMake.code.inputLocked.executing'
      : phase === 'failed'
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="flex min-h-[90px] w-full items-center justify-between gap-3 rounded-xl border border-[hsl(var(--sidebar-border))] bg-[hsl(var(--content-area))] px-4 py-3"
    >
      ...
      {phase === 'executing' && onStop && (
        <Button variant="primary" onClick={onStop}>
          {t('cindyMake.prepare.stop')}
        </Button>
      )}
    </div>
  );
