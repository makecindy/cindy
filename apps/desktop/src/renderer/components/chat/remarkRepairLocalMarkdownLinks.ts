...
function findMalformedLocalLinkMatches(value: string, rawSource?: string) {
  ...
  while ((match = MALFORMED_LOCAL_LINK_START_RE.exec(value)) !== null) {
    const rawIndex = rawOffsets?.[match.index];
    if (rawIndex !== undefined && rawIndex > 0 && rawSource?.[rawIndex - 1] === '\\') continue;
    ...
    if (/\s/.test(href) && kind !== 'external' && kind !== 'directory') {
      matches.push({
        start: match.index > 0 && value[match.index - 1] === '!' ? match.index - 1 : match.index,
        end: hrefEnd + 1,
        label: match[1],
        href,
        image: match.index > 0 && value[match.index - 1] === '!',
      });
    }
