...
describe('ContactsSection AI 管理入口', () => {
  it.each([
    ...
    expect(
      await screen.findByRole('button', { name: 'settings.contacts.guide.cta' }),
    ).toBeTruthy();
  ...
  it('点击后写入统一管理意图并进入新任务页', async () => {
    ...
    expect(mocks.prefill).toHaveBeenCalledWith(
      'settings.contacts.guide.managementPrompt',
    );
  ...
  it('功能关闭时不启动拿不到 cindy_contacts 的空任务', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: 'settings.contacts.guide.cta' }),
    ).toBeNull();
  });
});
