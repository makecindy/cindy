...
import { connectedProvidersForAgent, getModel, isChatEligible, visibleModelUnion } from '@cindy/model-providers';
...
export function SubagentModelSection() {
  ...
  const resolveProviderId = useCallback(
    (modelId: string, providerId: string | null): string | null => {
      ...
      return catalogModel && isChatEligible(catalogModel) ? providerId : null;
    },
    [providers],
  );
  ...
  const sourceDisconnected = Boolean(
    !providersLoading &&
      ...
      !connectedProvidersForAgent(providers, 'claude-code').some((p) => {
        ...
        return catalogModel !== undefined && isChatEligible(catalogModel);
      }),
  );
