...
export interface DeviceLinkCreateParams {
  /** 草稿 vendor 形态:'cc' | 'codex' | 'pi' | 'kimi'(persistedAgentKind)。 */
...
export function buildDeviceLinkCreateArgs(p: DeviceLinkCreateParams): DeviceLinkCreateArgs {
  ...
  return {
    agentKind: dbToMakerAgentKind(p.agentKind),
...
export function buildProvisionalRemoteSession(p: ProvisionalRemoteSessionParams): Session {
  return {
    ...
    userSendAt: p.nowIso,
    status: 'active',
    // Session.agentKind 是本机形态('cc' | 'codex' | 'pi' | 'kimi'),args 里是 maker-core 形态,这里转回来。
    agentKind: makerToDbAgentKind(p.args.agentKind),
