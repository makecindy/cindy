...
describe('父层 — 行级 handler 的引用稳定性', () => {
  ...
  it('SessionCard / ProjectNode / AutomationSessionGroupItem 必须 memo 包裹', () => {
    ...
    const navigationAdapter = readFileSync(resolve(__dirname, '..', 'sidebarNavigation.tsx'), 'utf8');
    ...
    expect(automationGroup).toMatch(/export const AutomationSessionGroupItem = withSidebarNavigation</);
  });
