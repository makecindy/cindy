...
export interface CollabEntryPolicyScope {
   ...
   * dialogue 没有用户选择的项目,自然只读用户级开关;SSH 远端会话的 workingDir 则是远端
   * 机器上的路径,拿它在**执行查询的那台机器**的 fs 上找项目配置既无意义又会误判
   * (命中同路径的本地目录会误开,查不到又会误拒)。远端项目级 collab 配置机制尚未存在
   * (main 侧 assertCollabProjectEnabled 的 remote 分支同口径)。
   */
 ...
 * | dialogue(无项目目录)              | ✅       | skipProjectQuery(仅用户/全局级) |
 * | Orca Worker 子会话                | ❌       | —                               |
 *
...
export function resolveCollabEntryPolicy(target: CollabEntryTarget): CollabEntryPolicyScope {
  ...
  if (target.workspaceKind === 'project' && nonEmpty(target.workingDir) === null) {
    return NOT_ELIGIBLE;
  }
  ...
  return {
    ...
    skipProjectQuery:
      target.workspaceKind === 'dialogue' || nonEmpty(target.remoteHostId) !== null,
  };
}
