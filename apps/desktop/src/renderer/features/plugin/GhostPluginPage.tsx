...
export function GhostPluginPage() {
  ...
  const handleRestorePluginRecovery = useCallback(
    async (candidateIds?: string[]) => {
      const proposal = recoveryStatus?.proposal;
      if (!proposal || candidateIds?.length === 0) return;
      setRecoveryBusy(true);
      try {
        const result = await window.electronAPI.pluginMarket.resolveRecovery(
          proposal.proposalId,
            ...
            <section className="plugin-motion-page-section mt-6 min-w-0">
              ...
              {recoveryStatus ? (
                <PluginRecoveryNotice
                  ...
                  onRestore={(candidateIds) => void handleRestorePluginRecovery(candidateIds)}
                />
              ) : null}
...
export function PluginRecoveryNotice({
  ...
  onRetry: () => void;
  onKeep: () => void;
  onRestore: (candidateIds?: string[]) => void;
}) {
  const { t } = useTranslation();
  if (status.state === 'none') return null;
  const proposal = status.proposal;
  const count = proposal?.totalCount ?? 0;
  const messageKey =
    status.state === 'review'
    ...
    <div className="mb-4 rounded-xl border-[0.5px] border-[var(--border-default)] bg-[var(--surface-elevated)] px-5 py-4 text-left">
        ...
        <ul className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
          {proposal.candidates.map((candidate) => (
            <li
              ...
              className="flex items-start justify-between gap-3 rounded-lg border border-[var(--border-default)] bg-[var(--surface-secondary)] px-3 py-2.5"
            >
              <div className="min-w-0">
                <p className="truncate text-13 font-medium text-[var(--text-primary)]">
                  {candidate.name}
                </p>
                <p className="mt-0.5 break-all text-12 text-[var(--text-tertiary)]">
                  {candidate.ghostId} · {candidate.version}
                </p>
                <p className="mt-1 text-12 leading-4 text-[var(--text-secondary)]">
                  {t(`settings.ghosts.recovery.reason.${candidate.reason}`)}
                </p>
              </div>
              {candidate.readiness === 'ready' ? (
                <button
                  type="button"
                  onClick={() => onRestore([candidate.candidateId])}
                  disabled={busy}
                  className="inline-flex h-8 shrink-0 items-center rounded-full border border-[var(--border-default)] px-3 text-12 font-medium text-[var(--text-primary)] transition-colors hover:bg-[var(--surface-hover-soft)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] disabled:cursor-wait disabled:opacity-55"
                >
                  {t('settings.ghosts.recovery.inline.restoreOne')}
                </button>
              ) : null}
            </li>
          ))}
      ...
      {proposal?.truncated ? (
        <p className="mt-2 text-12 text-[var(--text-tertiary)]">
          {t('settings.ghosts.recovery.inline.truncated', { count: proposal.totalCount })}
        </p>
      ) : null}
      <div className="mt-3 flex flex-wrap gap-2.5">
        {proposal && proposal.counts.ready > 0 ? (
          <button
            type="button"
            onClick={() => onRestore()}
            disabled={busy}
            className="inline-flex h-9 items-center rounded-full bg-[var(--confirm-btn-primary-bg)] px-4 text-12 font-medium text-[var(--confirm-btn-primary-text)] transition-colors hover:bg-[var(--confirm-btn-primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] disabled:cursor-wait disabled:opacity-55"
          >
            {t('settings.ghosts.recovery.inline.restoreAll', {
              count: proposal.counts.ready,
            })}
          </button>
        ) : null}
        <button
        ...
        {proposal ? (
          ...
          >
            {proposal.notificationMuted
              ? t('settings.ghosts.recovery.inline.muted')
