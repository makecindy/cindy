...
export function GhostPluginPage() {
    ...
    async (id: string, enabled: boolean, displayName: string) => {
      try {
        ...
        if (dir) {
          ...
          if (enabled && result.setupStatusUnavailable) {
  ...
  const runMarketInstall = useCallback(async (
    ...
    const diff = isUpdate
      ? diffInstalledGhostPermissionItems(installedGhost!, detail.manifest)
      : null;
    try {
      ...
      >
        ...
        <span className="flex min-w-0 flex-1 flex-col self-stretch pt-0.5">
          <span className="flex min-w-0 items-center gap-2">
            ...
            {/* 未批准时不叠就绪徽章:此刻唯一可执行动作是重新确认权限,
                再挂一个"需配置"只会分散唯一下一步。 */}
            {needsReapproval ? null : <GhostReadinessBadge readiness={readiness} />}
          </span>
