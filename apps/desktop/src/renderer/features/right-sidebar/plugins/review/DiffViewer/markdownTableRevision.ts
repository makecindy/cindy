...
function validateTableRevision(
  ...
  return (injectedTable.align?.length ?? 0) === (referenceTable.align?.length ?? 0);
}
...
export function buildMarkdownTableRevision(before: string, after: string): string | null {
  ...
  return validateTableRevision(injected, reference, rows.length) ? injected : null;
}
