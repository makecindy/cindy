 ...
 *
 * 输入框只写会话行的话,用户在对话里选的模型会在运行时重启或
 * 异常恢复后悄悄跳回去。因此 Profile 必须保持同一份持久选择。
 */
...
function capabilities(overrides: Partial<BotCapabilities> = {}): BotCapabilities {
  return {
    ...
    fastMode: false,
    harness: 'claude',
    modelChain: [{ harness: 'claude', model: 'claude-sonnet-4-6', providerId: null, effort: 'medium', fastMode: false }],
    skillMode: 'inherit',
    skillsExcluded: [],
    toolsetMode: 'inherit',
    toolsets: [],
...
/*
  ...
  下次运行时启动又跳回旧值,全程没有任何反馈。根因是 store 还没 hydrate 完时
