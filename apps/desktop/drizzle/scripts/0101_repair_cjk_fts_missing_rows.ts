...
function ftsIndexIntact(db: Database.Database): boolean {
  const duplicateFtsId = db
    .prepare(
      `SELECT 1
       FROM messages_fts
       GROUP BY message_id
    ...
    .get();
  if (duplicateFtsId) return false;
  ...
  const mismatchedFtsRow = db
    .prepare(
      `SELECT 1
       ...
       WHERE f.session_id IS NOT m.session_id
...
function rebuildMessagesFts(db: Database.Database): void {
  registerCjkSeg(db);
  ...
  if (!reuseExistingFtsRows(db)) rebuildFts(db);
