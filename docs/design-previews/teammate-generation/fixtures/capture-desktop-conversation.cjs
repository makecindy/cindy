...
const stubModules={
 '@/features/device-link/remoteProjectsStore':`export const remoteProjectsStore={getSessionDeviceId:()=>null};`,
...
(async()=>{
 ...
 try{for(const before of [false]){
  const stage='reference',bundle=path.join(root,'tmp/teammate-generation-components',`desktop-${stage}.js`);
  ...
  await esbuild.build({entryPoints:[path.join(fixture,'desktop-conversation-fixture.tsx')],outfile:bundle,bundle:true,format:'esm',platform:'browser',jsx:'automatic',loader:{'.png':'dataurl','.svg':'dataurl'},define:{__BEFORE__:String(before),'import.meta.env.PROD':'true','import.meta.env.DEV':'false'},plugins:[plugin],logLevel:'warning'});
  for(const scenario of ['conversation'])for(const theme of ['light','dark']){
   ...
   await page.goto(`http://fixture.local/?theme=${theme}&scenario=${scenario}`);await page.waitForSelector('button');await page.waitForTimeout(100);
   if(errors.length)throw Error(errors.join('\n'));
   await page.screenshot({path:path.join(themes,`desktop-chat-reference-${theme}${scenario==='compacting'?'-compacting':''}.png`)});
