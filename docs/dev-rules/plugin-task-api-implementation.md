...
Host 只接受经绑定插件身份写入的自有创建收据；已登记计划整体不可变，包括原本缺失的范围。
旧计划缺少范围时沿用原审批行为，不事后补造授权；新范围须在新任务中登记完整计划。
没有新增权限档位、没有提升到 Full access。
...
直接派发和排队派发均保存受保护的 `delegated-continuation` 作者凭据；应用重启后从同一 Host 收据重建委派范围。
