...
- 首次使用默认 `auto`（Auto-review）。UI 每次创建 Worker 都可选择 `auto` 或 `bypassPermissions`（Full access），提交后成为下一次默认值。
- MCP `start_team` 可显式指定 `worker_permission_mode`；显式值立即更新 main 镜像，并通知 renderer 回写同一份 localStorage。省略时沿用当前偏好。
...
- Team Worker 权限：覆盖 UI/MCP 未指定默认 `auto`、显式 `bypassPermissions`、复用 Team 时省略不重置/显式才更新、首个与后续 Worker 都读取 Team 默认、已有 Worker 权限不反写、重启后从 DB 恢复，以及旧 device-link 被控端被阻止开启协同并提示升级。
