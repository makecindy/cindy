...
7. **结束 team 必须选择性失效该 team 的自动消息（状态：不变量）**<br>
   ...
   共享 DB 多实例还要求 user 行在同一条 SQLite INSERT 中复核 team 仍为 active，并写入精确 `agent_meta.orcaPreVendorCleanup.teamId`。terminal reconciliation 只能按这个显式标记条件 rewind；dispatch lease 在本地 transport 已提交时与释放 writer lock 同事务删除标记，确认未提交或发现 team 已终态时则同事务 tombstone 精确 `{sessionId, clientId}`。这样 terminal sweep 等到 lease 后不会误删已提交消息，发送进程在持久化后退出也仍有跨实例恢复依据。
