...
这套机制只等价于无损 hibernate，不等价于 archive/delete。`idle_worker/archive_worker/end_team` 仍保留显式控制语义；其中 runtime release marker 只是这些操作完成最终持久化前的恢复护栏。`Session.close({ releaseRuntime: true })` 是比普通 resumable close 更强的关闭意图：两者并发时必须在 Session 进入 closed 前升级并补做 provider runtime release，普通 close 不能代替它报告成功。Codex `thread/archive` 超时后仍按失败处理，只有重试收到结构化且明确表示 thread 已归档的响应，或 app-server transport 已退出并确认 runtime 不再存活时，才允许完成本地关闭。长期 Worker 的自动有损回收，以及 persistent/ephemeral 分型后的差异化回收策略，仍应保持保守并在后续单独落地。
