...
12. **PI 后台命令由 Host 独占拥有（2026-09-18）**：`bash` 的 `background: true` 只是请求
   ...
   **正面证据**，所以显式传 `keepWhenUnowned: false`，否则崩溃残留永远收不掉。
   标记的含义是「**现在**还有本实例的命令在往这个目录里写日志」，所以它跟着状态走：`start()`
   时写，`running` 与 `pendingKills` **都**清空时（最后一个命令收口、且没有已收口但未确认退出的
   残留进程）撤销 —— 只写不撤会让一个早已用完这个会话、进程还活着的实例把标记永远挂在那里，
   会话删除侧就一直判定「别的实例在用」，目录再也没人回收（启动期只扫 `anon-*`）。写与撤共用
   一条串行队列，避免「刚起就停」时先写的盖掉后撤的。已知残留：删除会话时另一个实例**正在**
   跑该会话的命令（`kept-foreign-owner`）时，那个目录要等后续的会话日志 TTL / 配额工作来收。
