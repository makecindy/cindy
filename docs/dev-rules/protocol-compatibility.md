...
## Model Registry：服务端发布与旧端下发

- 当前客户端请求 `/api/model-catalog/catalog?registrySchemaVersion=5&catalogCapabilities=server-managed-catalog`。供应商、接入、Registry 与 Pi 参数来自同一份服务端数据库发布；离线只读同区域、同来源的上次有效快照，没有随包 JSON 补齐。
- `nativeApi` 与窗口、参考价和输出上限在 Platform WebUI 维护。`nativeApiRules` 仅按指定 providerId + modelIdPrefix 匹配；精确声明优先，显式 null 表示待核实，退役项禁止继承规则。跨厂商不根据同名猜测。
- 服务端删除条目或字段后，当前客户端不恢复旧内置值。修订号、LKG、同版本冲突拒绝与来源隔离仍生效；用户显式连接与参数覆盖保持独立。
- 服务端按声明能力投影：无能力参数的 V5 保留其已有媒体契约，但不接收新增 Pi 参数目录与工具能力字段；V4 及更早版本沿用相应降级。ETag 按实际响应格式隔离。禁止直接向严格旧客户端返回新增字段。
- 首版数据库来自客户端 main 与服务端 main 的实际合成结果。初始化 JSON 仅用于空库导入，上线后冻结；后续修改通过 Platform 保存草稿并发布。旧客户端仍保留它自己随包的数据，完整撤销语义需要升级客户端。
- UI 的 CatalogModel.nativeApi 是主进程按实际 provider/model 投影的结果。Pi 的 piApi 是执行配置，不能反过来当作模型原生协议。界面只展示简短协议名与兼容模式。
- 默认开关按原生协议与 Harness/出站协议比较：兼容路径默认关闭；用户显式偏好优先，服务端逐 Harness 的 defaultEnabled 保留覆盖能力。协议未知时不猜测。
- 上线前分别核验服务端实现、客户端消费和部署状态；验证旧客户端降级、新客户端完整目录、更新/撤销/冲突及 ETag，不能仅凭仓库改动宣布线上生效。

协议参数以服务端发布的 `providerModelCatalog` 为数据来源，官方 SDK 提供协议实现。
Gateway 的 `perAgent.pi.wireProtocol` 仅是末级执行提示，不能覆盖 Registry 原生协议或填充 UI 字段。
