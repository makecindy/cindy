...
以下缺口在触及相关链路时必须一并修复，或在 PR 中保留明确的正式跟踪，不得静默丢弃：
 ...
 - 插件持久引用的 per-插件 字节配额目前只覆盖两类专项能力：
   - **寄存**（`ghost-deposit`，cindy 槽 `deposit_media`）：上限
     `GHOST_CINDY_DEPOSIT_QUOTA_BYTES`，安装确认框逐项展示该上限，释放口是
     `release_media`，卸载插件时按 refKind 清理。
   - **appearance 皮肤预设**：按插件限制为去重后 128 MB，并在安装／更新确认中展示该
     上限。
   **画廊（`ghost-gallery`：模型代办产物与 network `as:'media'` 下载）仍无字节配额、
   仍不随卸载回收** —— 这两条是存量语义，改动它们属于产品决策，触及时另行拍板，不得
   当作已完成。后续新增其它插件持久引用仍必须补对应配额与确认文案，不能把上述专项上限
   误当成通用框架已完成。
