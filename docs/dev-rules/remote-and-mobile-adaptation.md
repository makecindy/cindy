...
`CHANNEL_NOT_ALLOWED` → 按钮保留但请求失败：两侧 UI 都不做乐观收口（行仍显示 running，
按钮留在原地可重试）；本分支的失败是静默的（面板/卡片只 catch，不弹提示），更明确的失败
文案属于后续 PR。反向不成立：状态栏的「后台任务运行中 / 全部停止」计数仍只服务本机会话
（镜像事件有设计内丢失窗口，计数不可作权威）。
