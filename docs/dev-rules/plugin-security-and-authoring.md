...
- 自动对账在 owner 稳定后的启动／登录切换、应用回前台、系统唤醒和 30 分钟周期触发。
  ...
  后台先对账官方目录与已有缓存，再刷新距上次成功同步至少 30 分钟的自定义 Git 来源，
  成功后重新发现并沿原安装策略对账。普通页面快照不拉取 Git；本地文件夹继续直接发现。
  Git 刷新失败保留旧缓存，按 owner、来源及添加批次在进程内从 30 分钟起退避、最长 6 小时；
  手动刷新不受退避影响，成功后清除该添加批次的失败预算；来源删除立即清理预算，
  每轮后台刷新仅保留当前 owner/generation 的现存来源记录。真正的 Git 拉取在已有缓存对账之后
  异步执行，认证初始化不等待它完成；后台异常被捕获记录，仍保留周期重试。
  刷新复用来源锁与版本目录事务，慢仓库期间其它来源操作仍可能排队。
