...
| `ghost.json` 身份卡字段与校验                        | `packages/plugin-protocol/src/manifest.ts` 是跨消费者协议正本；`apps/desktop/src/shared/ghost.ts` 是 Desktop 必须同步维护的完整协议镜像与运行时 validator，不是 re-export                                                                                                                                                                                                                                                                                                                  |
