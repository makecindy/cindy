...
- `appearance apply` 表示整套替换；`appearance patch` 表示微调当前皮肤。patch 未提供
  ...
  普通主题与插件皮肤是互斥的外观来源：皮肤以 Cindy 对应 Light / Dark 变体为固定基础，
  不得叠加用户此前选择的内置／本地主题；显示模式只选择当前皮肤自己的明暗变体。
  patch、save-current 与 reset 只能操作当前插件自己提供的活动皮肤，不能读取、接管或停用
  其他插件的皮肤。
  保存 patch 时必须继续保留未修改媒体的业务引用，不能因请求只带一张头像就释放背景或 Logo。
  带名称的皮肤可保存为账号级预设；预设媒体使用独立持久引用，切换皮肤或恢复默认不得
  回收。每个插件的预设媒体去重后上限为 128 MB，安装／更新确认必须展示该持久占用上限。
  插件只可列出、激活、更新和删除自己名下的预设；可信宿主设置页可以统一管理
  全部预设。插件不得自行保存宿主 URL 或绕过槽读写外观文件。
