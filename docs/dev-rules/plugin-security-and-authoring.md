...
- 附件、媒体、目录和保存路径通过归属校验后的 grant／deposit／ledger 交接，**禁止把
  ...
  cindy-docs / 电脑越界路径在弹出确认前就钉住祖先身份，确认返回后必须对上；
  父目录被换成链接或另一个普通目录后不得跟读跟写。
  Forge 的 `isCurrent` 必须传到 `.cindy` 写入、骨架落盘和安装／发布事务，不能只在入口检查；
  确认前后也要核对源码目录身份。
  电脑工具同样把授权代次带到派发与轨迹回放的每一步，权限失效后不得继续写或重放。
