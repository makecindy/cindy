...
- **安装／更新确认**（`shared/ghostInstallConsent.ts` 判据，`main/cindy-brain/ghostInstallConsent.ts`
  ...
  - 两段式：确认在任何安装锁、来源锁与 owner 租约之外求得（等待期间不阻塞其它插件）；
    ...
    （`assertGhostInstallConsent`），确认后权限面、已装版本、受体或包字节变化就拒绝；
    用户确认过的更新不能因为受体已被换成权限已覆盖候选包的版本而静默落位。Forge 在
    packing 租约内捕获 owner 并传入装入入口，等锁结束后若账号已切换则拒绝取租约。每条
    装入路径都必须显式交出确认策略（`prompt`／`automatic`／`exempt`）或锁外求得的结论，
    签名上必填，新增路径无法漏掉。
...
- 仍匹配稳定来源的已装插件自动更新，不限 public／organization；更新保持现有启用状态，
  ...
  `updateRequiresConsent`，插件页标出「需确认新权限」，用户点更新时再弹确认。自定义
