...
- **锚点覆盖判定以「裁剪后真正上报的记录」为准，不是读到的全部**（`buildFileCoverage`）：多次
  ...
  清除，不会无限重传。
- **锚点覆盖不能只看留下记录的 `[min,max]` 跨度**（`fileCovers`，2026-08-06 review）：同日多次崩溃时
  cap 可能只留住最早与最晚崩溃附近的记录、把**中间**那次的记录全裁掉，中间锚点仍落在跨度内（被两端
  「架桥」）却没有自己的现场进上报。判覆盖要**同时**满足「A 落在跨度内（读取确实推进到 A 两侧）」与
  「A 的邻域内有记录留下（自己的现场没被裁光）」：前者挡住「读到一半停下」（污染停止点在 A 之前），
  后者挡住架桥。
