...
Mobile 的设置 → 调试 / 开发者提供本地记录、导出、清空与手动上传。普通构建默认不记录；
个人排障构建可用 `EXPO_PUBLIC_CINDY_DIAGNOSTICS=1` 设置默认值，用户保存的开关优先。

- `src/debug/diagnosticEvents.ts` 是记录与读取共用的白名单：只重建固定事件名、有限数值和枚举，
  不保留原始 console、错误正文、堆栈、地址、凭证、消息正文或设备/账号标识。导出与上传再次投影。
- 本地 AsyncStorage 日志最多 500 条、保留最近 7 天；记录关闭不自动删除已有日志，清空单独操作。
  分享只生成一个缓存 JSON，分享结束删除。不扫描其它文件，也不接管全局 console。
...
- Metro 从 `config/log-upload.json` 只注入当前构建区域的 `EXPO_PUBLIC_CINDY_LOG_UPLOAD_TARGET`。
  本地/开源缺少文件时隐藏上传入口；正式发布环境需同步该私有配置并设置
  `CINDY_REQUIRE_LOG_UPLOAD_CONFIG=1`，避免漏配时静默关闭。损坏配置或区域交叉配置始终硬失败。
