...
- 当前纯函数只在 host 明确提供 `windowsCaseComparison: ordinal-insensitive` 时对 ASCII Windows canonical path 做分隔符、扩展长度前缀和大小写归一化；若 host 报告 `case-sensitive`，则保留 canonical path 大小写；缺省、`unavailable` 或无法证明比较语义时统一 fail closed。`ordinal-insensitive` 下的非 ASCII Windows path 因 JavaScript Unicode folding 不等同于 Win32 ordinal comparison，也必须 fail closed 为 `unavailable`，直到 host 提供独立的 Win32 comparison identity。
...
- `skills`：只有调用方显式证明 `explicitSkills: true`，并通过 pinned Pi v0.83.0 事实夹具验证后，才可用单独 skill 路径装配；能力位缺省为 false。它不代表 Pi 已经 `loaded` 这些资源；
