...
| Bot 能力自知与实时发现 | `botProfileRuntime.ts` 的独立能力上下文、`cindy_helper` Bot 工具类目 | `botProfileRuntime.test.ts`、`lizi_xdtHelperMcpServer.test.ts` | 新建与预置人格都自动知道自己是持久 Bot；回答“不支持”前先通过 `list_tools` 检查实时能力，不在提示词里穷举具体工具名。能力说明明确支持发现其它 Bot、转交有界任务、自动接收回传、查询和取消；“服从”请求会说明人格边界并给出委派路径，不把用户赶去协同 team。 |
...
| `@Bot` 一等输入入口 | 标准 `ChatInput` / `AtMentionPanel` / `MentionChipNode` 与 `AgentInputReference.kind='bot'` | `composerSuggestion.test.ts`、`composerStructuredLists.test.ts`、`agentInputProjection.test.ts`、`sentAgentReferencePresentation.test.tsx` | Bot 主任务输入 `@` 会列出其它活跃 Bot；选择后保存稳定 Bot ID 与名称，历史、队列和重试保持结构化引用。Agent 侧收到“委派或接力目标”语义和非服从边界，再走实时发现到既有 durable delegation 链，不靠显示名猜目标。 |
...
| Desktop UI | `renderer/features/bots/**` | renderer store、`botNavigation.test.ts`、typecheck、设计契约测试 | 左侧 Bots 是固定任务入口；主区域复用真实任务与 ChatInput；设置、历史、委派、worktree、Automation 和投递诊断均有入口。Bot 任务加载失败保留明确错误、诊断、重试和返回入口，不再静默跳走；收件匣、Automation 和投递错误的长内容可滚动看完整文本，截断标题保留原文 tooltip。 |
