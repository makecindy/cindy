...
| 新 Bot 数据 | 新代码 | Bot Profile、通道、Route、主任务、历史、任务事件 ledger、逻辑订阅、收件匣、自动化、委派、worktree lease 与 outbox 按各自表运行；旧 IM 表仍作为 adapter 数据面存在，不被搬入 Bot 表。 |
...
- 0092 增加 Bot 主体、生命周期、能力与迁移表；0093 增加任务事件 ledger、逻辑订阅和 Bot 收件匣。两者及后续 Bot migration 保持只增量；不得 `ALTER`、`DROP`、`UPDATE` 或 `DELETE` 旧 IM 表。
