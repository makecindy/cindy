...
- **任务卡**：发起方消息流原位出现任务卡，实时反映排队 / 执行 / 等待 / 终态；执行者是伙伴时
  对方主任务里有镜像卡。所有 call 都是独立任务，卡片不得使用“加入对话”等通讯语言；执行者
  只是任务属性。卡片由 delegation 行推送驱动，不靠消息文本。
...
`notify` 是另一条更轻的伙伴间通讯通道，不得伪装成任务：

- 点名伙伴的简短询问、讨论与信息传递走 `notify`；开发、修改代码、制作网页或文档等需要独立
  交付和验证的工作不能走 `notify`。用户明确点名执行伙伴时用 `call`，未点名时用
  `start_session_task`。
...
- `call` 与 `start_session_task` 都能：任务卡实时更新到终态、挂起交互回到伙伴代答、完成
  信号对用户不可见、父卷换代后
  仍投递到当前卷。
