...
| Claude Opus 套餐终态的**请求级归因与安全文案** | `maker-host/claude-gateway-error-observer.ts` + `im/shared/turnRetryNotice.ts`（`terminalErrorText`） | Gateway / Claude.ai subscription 的 request evidence 在 Session 监听器 fan-out 前只消费一次并附到同一个 terminal event；个人侧 `turnRunner.ts` 与官方侧 `hook-control/turnObserver.ts` 都按 stable reason 调同一份安全文案映射，不再从原始错误串猜路由。两侧仍保留下一节既定的不同收口外壳（个人保留过程区，官方走独立 `errorMessage`）；原始诊断错误只留在本地事件／日志，不作为这两类失败的用户文案 |
...
| **失败收口**（普通轮次） | **过程区保留**：错误路径不置 `turn.done`，`composeStreamingView` 仍走运行中合成——卡片定稿成「过程区 + 正文 + ❌ 错误：…」，用户能看到失败前干到了哪一步。Claude Opus 两类 request-attributed 套餐终态的「…」走第一节登记的共享安全文案 | 终稿正文为**空**，错误信息走独立的 `errorMessage` 字段，由服务端按语言渲染成「任务失败：…」——**不带过程区**。Claude Opus 两类 request-attributed 套餐终态的 `errorMessage` 同样走共享安全文案 |
