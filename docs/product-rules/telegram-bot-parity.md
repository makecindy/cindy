...
| 5 | **媒体失败后的回落与重试判据尚未同源** | 个人 bot 已核：相册在本地组装失败或 Telegram 明确 4xx 拒绝时逐张回落；网络错误、5xx、429 属于结果不确定，不补发并记为不可安全重试。单图成功才确认送达；网络错误、5xx、429 记为不可安全重试；明确 4xx 拒绝不计入收口进度，保留安全重试并让终态失败。官方 bot 的对应判据仍在服务端独立实现，尚未逐分支核对 | 官方侧待核；在核清并抽出跨仓契约前，不得把个人侧的确定拒绝改成已收口，也不得对结果不确定的相册或单图自动补发 |
