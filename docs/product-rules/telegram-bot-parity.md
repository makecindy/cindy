...
| 5a | **工具生成视频的附件回流只有官方 bot 支持** | 官方桌面端从 `tool_result_full` 的 `xdt_video_urls` / `xdt_media_produced` 接走 `xdt-video://` 与 `cindy-media://` 视频，按既有附件数量、单文件与总字节水位随 `turn.end` 回传；普通轮次与 continuation 共用同一收集逻辑。个人 bot 的 `im/shared/turnRunner.ts` 会识别这些字段，但当前明确只记日志并跳过视频上传 | 个人侧仍是缺口。本次只修 #2801 的官方 hook 静默丢视频，不顺手改个人 bot 的出站实现 |
