...
| 2g | **交互卡挂太久：官方 30 分钟自动收口，个人原生卡一直等** | 三类卡（`ask_user_question` / `plan_review` / 权限）在两侧走的是同一个注册入口，但超时来源不同。官方：`hook-control/interactions.ts` 的 `registerHookInteraction` 给每张卡挂一个 `HOOK_INTERACTION_TIMEOUT_MS = 30min`，到点取共享模型的安全默认（ask 空答 / plan deny + dismissed / permission deny）resolve，并回调 `onFallback` → `session-runner.ts` 的 `sendCancel(requestId, reason)` 把卡片收掉。个人 bot **原生创建**的三类卡仍无定时器：`im/shared/pendingInteractions.ts` 只能靠按钮决策（`resolvePending`），或 turn 收口 / session 清理 / 抢跑时的 `dropInteractionCard` → `cancelPending`（安全默认与官方同源，并把卡片改成「卡片已过期」），所以原生卡会一直挂着，第二天点还能点，agent 也还在等。**唯一例外是 Desktop `/ctr` 迁移过来的权限卡**：它沿用 Desktop 已开始计算的 10 分钟绝对截止时间，只消费迁移时剩余的时间，到点安全 deny 并把卡片收口；迁移不会把倒计时重置，也不会给个人 bot 原生卡引入统一超时。maker-core 的 turn stall 看门狗仍明确把「等用户回应交互」排除在静默之外。 | 待判，**不在本 PR 统一原生卡超时**。先答一个产品问题：个人 bot 的原生卡挂着不动算不算问题——它是用户自己的 bot，晚点回来再点也说得通；官方 bot 经服务端中继，挂着的交互会长期占住 lane，动机不一样。补的话要连「超时后卡片显示什么」一起定。Desktop `/ctr` 迁移权限沿用原截止时间只是保持接管前后的同一次权限请求不延寿，不改变这项待判裁决。**顺带**：官方那个 30 分钟的注释理由（「必须短于整 turn 硬超时 60min」）已过期——那条硬超时 2026-08-01 撤了，定时器本身仍在生效，注释已随本 PR 改正 |
