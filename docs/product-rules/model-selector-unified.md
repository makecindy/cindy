...
- 结构:搜索框贴顶 → 左侧 rail(最近 / ★收藏 / 全部 / 各供应商图标) → 右侧列表 → 底部「＋配置模型」。
  「最近」格在 ★ 之上（2026-09-16 裁决），与 ★ 一样常驻，图标用时钟回弹箭头（History），点击进入**独立的最近视图**；空列表时显示引导空态。常规视图（全部 / 各供应商 / 同引擎）**不**陈列最近区（与收藏上下叠放会产生视觉歧义）。
...
- 行(双行):
  ...
  - L2:一句描述(截断)。收藏 / 最近 / 全部视图的 L2 改显 provider 来源(来源详情行,同名模型跨来源价格不同);仅单来源视图显描述。
  - 三元组**所有行同构、永远显示**——引擎可见性靠一致的结构位,不靠出错才显示;自定义未收藏的行整组提亮一档(secondary)。
- 分组:收藏 → 推荐（仅已有任务）→ 供应商分组。最近**不是**常规视图里的一区（2026-09-16 实测裁决）——它只在侧栏「最近」格（位于收藏之上，见上一节 rail 说明）单开一页，不参与本行顺序。推荐区先放当前模型，再放同生效引擎的模型；推荐中的模型从下方供应商组移除，空组不渲染标题或间距。供应商侧栏仍基于完整目录。新任务不展示同引擎推荐。组间顺序遵循「设置 → 模型供应商」的拖动排序(本地会话;device-link 用被控端快照顺序,不套控制端本地序),组内按服务端下发 group/sortOrder。收藏区条目**不**从供应商组中去重移除(收藏是配置副本,模型本体仍在原地)。
- **最近视图（2026-09-16）**:只由侧栏「最近」格进入，按最近一次使用时间倒序陈列最近 5 条**配置副本**（与收藏同一套语义:来源 + 模型 + 引擎 + 深度 + Fast;同一模型不同配置各占一行），行与收藏视图一致(L2 显 provider 来源)。点击一条按该副本的完整配置应用；已标星的行与收藏行同构（带上那条收藏的 uid，选中锚点 / 星标一致）。被收藏的副本标星——先按**来源 + 模型**圈定同一条目，再比解析后的引擎 / 深度 / Fast 一致即点亮（只看三元组会跨型号误命中，点星删错收藏）；未收藏点星把这份副本存进收藏，已点亮再点星从收藏里移除。**取消收藏若删的正是当前选中的那份收藏**，与收藏视图完全同一口径：先把该模型的默认配置应用出去再删（否则正在跑的配置与显示分家）；这是「删收藏」的通用语义，最近视图不另开一套。该视图是只读流水账：**不提供配置浮层**（改配置走模型行或收藏）——最近行不绑右键 / ←，也不宣告 `ArrowLeft` 快捷键（右键不抢原生菜单）。记录点只有对话里「选择真的应用成功」的那一下(新任务草稿 / 已建任务 / 任务内切换)，记的是那一刻生效的整份配置;定时任务 / IM 默认 / Bot / Hook / Worker / 子代理与设置页的选择是配置动作，不记;SSH 与 device-link 远程也不记（被控端目录的模型在本机不一定可路由）。不可路由的条目不删只隐藏，数量不足时从更早的历史补足。首日为空是预期状态，不做事后回填。
...
4. `xdt:recentModels:v1:<dataOwnerId>` —— 「最近」**配置副本**流水:
   `{ items: [{ providerId, modelId, agent, effort?, fast?, usedAt }] }`(usedAt 倒序,本机保留
   `RECENT_MODELS_KEEP = 20` 条,视图陈列前 5)。字段归一化与去重身份与收藏**共用同一份**
   (modelConfigCopy):同一模型不同配置各占一行;跨窗口按副本身份取 `max(usedAt)` 并入
   (与前三项同一套 op-log 调和,记录 op 幂等且无删除类)。记录写入在对话入口「选择真的应用成功」的回调里：选择是异步的(跨引擎事务 / 远程写)，回调落地时若归属已变(登出 / 切本地模式)则整条丢弃，不写进新分区；写基底 = 磁盘真相 ∪ 本窗内存态 —— 上一次写失败(配额 / 私密窗口)时那条记录只活在内存里，不能被下次 `freshState()` 读到的磁盘旧快照顶掉。
