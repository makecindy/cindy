...
余量读取复用现有账号缓存，按连接 ID 隔离。只取明确的周窗口，不把短窗口或模型专属额度当账号周限额；未取得数据、断开连接、窗口已结束、API 计费供应商不显示横线。远程目录不读取控制端本机余量，改读被控端的余量：与会话底部用量 chip 共用同一份被控端镜像，打开选择器不为 chip 已在展示或刚读取过的账号重复查询。Anthropic / xAI 当前原生账号读取接口仅适用于各自内置连接，不能借给同品牌自定义连接。
