...
- **A 版是新旧用户唯一的模型选择面板**（2026-09-05）。删除 B 版与样式切换入口，不再读取历史 `xdt:modelPickerLayout:v1` 偏好，升级无需手动切换。仅保留固定引擎表单及老被控端 capabilities-only 的兼容列表。
...
- 不再随 hover 自动弹出或收起。行 hover 时收藏 ☆ 右侧出现「自定义」(SlidersHorizontal),点它或右键该行打开;键盘入口仍是 ←。点击打开后，经过其他模型、离开行／浮层、焦点移动或滚动不收起。点击浮层外、Esc 或锚点被过滤／删除才关闭；主动打开另一行切换配置对象，Esc 后焦点回到原行。
