...
| 侧栏与移动端的区域标签 | `apps/desktop/src/renderer/components/sidebar/UserInfoSection.tsx:50` 对 Global 构建显示 `Global`、否则显示 `CN`；`apps/mobile/src/settings/mobileSettings.ts:124` 的 debug 项同样输出 `Global` / `CN` | §2.3、§2.5（`CN` 也不是本文规定的 `Mainland China`） | 属于调试 / 身份自查信息而非产品叙事表达，收敛优先级低于登录页，但不应在收敛登录页时被遗漏 |
