...
| | 是什么 | 能不能改 |
| --- | --- | --- |
| **管道**（永久规则） | 值只有一个出处；旧 Token ID 不删不改名；用户主题不改写磁盘；颜色走语义 token 不许硬编码；台账只有一份 | **不许绕过**——它与长相无关，改版后依然成立 |
| **记账**（当前拍板值） | 具体色号、圆角三档的当前成员、保护区观感（CINDY 皮肤族、登录链路、语义豁免色） | **随时能改**——走下面的合法改值路径 |

**守卫红灯不是禁令，是提醒走合法路径**：改数值真相源 → 同一 PR 更新对应守卫快照 /
台账 / 文档 → 按 §6 交证据 → 设计师批准。这是必要步骤，不是绕守卫。唯一禁止的事：
为了让红灯变绿而加豁免、绕加载路径、在别处重声明——那是砸管道，不是改设计。系统对
「混乱」的全部定义：消灭**没人决定过的变化**；一切拍板过的值都可以再拍板。
...
| # | 标题 | 风险类别 | PR |
| --- | --- | --- | --- |
| DS-1 | `docs(design-system): DS-1 建立治理合同与存量门禁处置表` | 零视觉（纯文档） | ✅ #3609（2026-08-29 合入） |
...
1. **棘轮管不到内联样式与非 CSS 绘制**——`style={{...}}` 的 JS 字面量、canvas / xterm
   一类自绘内容不经过样式文件，机器扫不到；靠 review 与 §11 登记的
   `check-pr-design-basis` 必读要求约束；
2. **「复用道路唯一」是纪律不是机器闸**——防止出现第二套 Button / Toast / 菜单动作模型，
   靠 review 裁决，机器只能发现雷同、不能自动判定谁该让位；
3. **语义分层是渐进的**——存量代码混引语义层与原始值是登记在案的现状；新代码优先语义层，
