...
- **2026-08-03（排版立法：字重四档 + 桌面字号白名单，issue #1505）**：`DESIGN.md §3` 字重梯由「仅 400/500」修订为 400/500/600/700 四档（600 = 限量强调；700 仅限豁免域；800 及中间值全禁；与手机端 `fontWeight` token 对齐），并建立桌面字号白名单 `{10–16,18,20,24,28}`、精确豁免登记与 non-goals。PR #1553 已完成 numeric 规范正文与三个 numeric 实现来源（Tailwind numeric 配置、CSS numeric 默认值、运行时 numeric 注册表）同步，并同步 `tailwind-merge` numeric 消费端；配置保留 `xl..5xl` 语义映射，由源码守卫拒绝使用并避免 `theme.extend` 删除后的静默缩放脱钩。
