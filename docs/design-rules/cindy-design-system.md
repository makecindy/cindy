...
- **2026-08-03（排版立法：字重四档 + 桌面字号白名单，issue #1505）**：`DESIGN.md §3` 字重梯由「仅 400/500」修订为 400/500/600/700 四档（600 = 限量强调；700 仅限豁免域；800 及中间值全禁；与手机端 `fontWeight` token 对齐），并建立桌面字号白名单 `{10–16,18,20,24,28}`、精确豁免登记与 non-goals。PR #1553 已完成规范、Tailwind 配置、CSS 默认值、运行时缩放注册表和 tailwind-merge 字号组的五处镜像，并以守卫持续校验。
