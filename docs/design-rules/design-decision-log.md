...
- **07-29** **手机端功能区落位改回新稿标注值(拍板人 = 用户,依据 = 实机比例对照图)**——
  ...
  **决策(方案 B)**:`loginY` 取新稿标注值 **622 / 827**(figma `705:915` / `705:799` 的
  ...
  (dh=1000 压 40 设计px),因为短屏档那 90 的底距只属于 dh≥1334,窄屏空间不足时不该保留。**已知偏离**:新稿手机帧面板为 500 高
  (含「跳过登录」栏),手机端已剥离该入口、面板回 440、组回 560,少掉的 60 **全部落到底部
  留白**(90 / 175,稿内为 30 / 115)。
...
- **07-28** **手机端「跳过登录」与无账号通路整体剥离(拍板人 = 用户)**——本条**修正**下方
  ...
  **保留的手机端视觉改版**(与跳过登录无关,本轮唯一保留项):短屏 / 长屏品牌簇换新稿
  ...
  〔**该决定已于 2026-07-29 撤销** —— 见下方 07-29 条:品牌簇换稿 + 落位留 main 拼出
  92 / 131.65 的空白,落位已改回新稿标注值 622 / 827〕。
