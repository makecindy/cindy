...
- **Low-frame-rate principle for persistent sweeps (2026-08-07)**: looping sweep/progress indicators (`ghost-card-sweep`, `session-card-bar`) use `steps(N, end)` easing tuned to ≈24fps (N = duration-in-seconds × 24) instead of smooth 60fps interpolation. Halves compositor work for animations that run for minutes, and the faint stepped cadence reads as paper grain rather than glass gloss — visually near-lossless. Apply to new persistent translate/sweep loops; do **not** apply to one-shot enters/exits or opacity breathing (steps on a 2-keyframe opacity fade collapses it into a blink).
...
- **Stream-word fade(流式正文逐词淡入)** — the fifth sanctioned motion class (approved 2026-08-07), **only** for streaming assistant prose in the chat message flow:
  ...
  - **Architecture red lines**: CSS owns the form, JS owns only the timing — `rehypeStreamWordFade` wraps words in `span.stream-word` and writes a per-word `--wf-delay` (16ms/word stagger per parse tick; backlog past 96ms compresses to 4ms/word as backpressure); the animation itself is a pure-opacity one-shot keyframe in `globals.css` (compositor-only). Words must never replay on re-parse (delay assigned once per document word index). The final (non-streaming) render carries **zero** wrapper spans — the plugin only mounts while `isStreaming`.
