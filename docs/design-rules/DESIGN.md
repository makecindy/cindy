...
### Settings source-link text action (#3597)

- `FeishuBotSection`'s "open the Feishu open-platform console" hint is a frameless text button registered as a narrow exception: minimum 24px hit height, 1px horizontal padding offset by a compensating negative margin, 4px radius only to shape the visible `--focus-ring`, link-colored underline in both modes. It is the recovery step of the QR setup flow, not a commit action, so it keeps the quiet text treatment instead of a pill; new uses must re-register here rather than inherit this entry.
