...
- **Mobile-download QR brand edge** — the fourth sanctioned motion class and the only sanctioned *persistent* one (approved 2026-07-25), **only** for the QR card in `MobileDownloadDialog` (brand-surface registration: §15.7):
  - **Definition**: the official app icon sits scaled behind the QR card and is clipped to a 1px hairline edge; it turns a full circle every **20s** (`--mobile-download-edge-cycle`, linear, infinite), so the artwork's own red/blue drifts along the card edge. The dialog is a promotion surface for the mobile app — the slow drift is what marks the QR as "ours" without adding any component-authored color.
...
- **Where**: `components/sidebar/MobileDownloadDialog.tsx` only — the dialog header icon (64px `resources/icon.png`) and the QR card's 1px edge, where the same asset is scaled behind the card and clipped to a hairline so the artwork's own red/blue reads as the card's edge. This is a promotion surface for the mobile app, so showing the app's own icon is identification, not decoration.
- **Constraints**: the color comes only from the official asset — no component-authored gradient or brand hex; the edge layer carries no `filter` / `backdrop-filter` / `mask-image`; its motion is governed by §14.4 below. The card itself stays flat: **no shadow and no pointer 3D tilt** (both were tried and removed 2026-07-25 — they are not part of this exception).
