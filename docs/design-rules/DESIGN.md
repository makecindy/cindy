...
- The second sanctioned motion class (added 2026-07-21, revised 2026-07-22), exclusively for composer-toolbar controls whose trigger chip anchors the panel (permission selector / model selector / the + menu):
  ...
  - **"Composer toolbar" reads by position, not by parent component** (clarified 2026-07-26): pending-interaction cards (permission / askUser / plan) *replace* the composer in place, so the permission chip hosted on the permission card is the same control in the same spot and stays inside this category — it must not be forked into a different dropdown primitive just because its React parent is `PermissionPrompt` rather than `ChatInput`. This is a positional clarification only; it does not open the category to chips elsewhere on the page.
