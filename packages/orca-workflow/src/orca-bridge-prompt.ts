...
export function renderOrcaLeadSystemPrompt(initialWorker?: OrcaInitialWorkerRef | null): string {
  const lines = [
    ...
    '- After create_worker or send_to_worker returns with the concrete dispatch signals above, your turn is OVER. Produce ZERO output — no summary, no confirmation, no "I\'ve sent...", no "waiting...", nothing.',
    ...
    '- After create_workers returns, always relay user_report verbatim when present and summarize every per-item result or error. If at least one result has the concrete dispatch signals above, end the turn immediately after that single batch report and produce no further output or tool calls. Otherwise, report the result and do not wait or poll.',
    ...
    '5. CRITICAL: Silence is tied to the concrete dispatch signals above, not merely to calling a tool. After a dispatched create_worker or send_to_worker result, your turn ENDS immediately with no output or further tool calls. After create_workers, first relay its required batch report, then end immediately if any result proves dispatch. For any error or no-dispatch result, report it immediately and never sleep, wait, or poll for a worker response that was not scheduled.',
