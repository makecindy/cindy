...
function createControllableHandle(opts?: { sendError?: Error }) {
  ...
  let interactionResolver:
    ((req: InteractionRequest) => Promise<InteractionDecision>) | null = null;
  ...
  return {
    ...
    async emit(e: AgentEvent, opts: { keepRunning?: boolean } = {}) {
      if ((e.type === "done" || e.type === "error") && !opts.keepRunning)
        turnRunning = false;
      if (push) push(e);
...
describe("Session per-turn origin 打标", () => {
  ...
  it("already-aborted send does not wait for a host turn lease", async () => {
    ...
    await expect(
      session.send("cancelled", { signal: controller.signal }),
    ).resolves.toEqual({
      accepted: false,
  ...
  it("handle.send 抛错(dispatch 失败)→ 清掉本次乐观 origin,不污染后续(别的 turn)事件", async () => {
    ...
    await expect(session.send("go", { origin: SCHED_ORIGIN })).rejects.toThrow(
      "boom-dispatch",
    );
    ...
    await emit({
      type: "text",
      data: { text: "别的 turn 的事件", isFinal: true },
    });
  ...
  it("失败 send 还原(而非清空)正在跑 turn 的 origin —— turn1 的 done 仍带 origin(回归 Greptile P1)", async () => {
    ...
    const ORIGIN_1: SendOrigin = {
      kind: "scheduler",
      scheduleId: "s1",
      scheduleName: "turn1",
    };
    const ORIGIN_2: SendOrigin = {
      kind: "scheduler",
      scheduleId: "s2",
      scheduleName: "turn2",
    };
    ...
    await expect(session.send("go-2", { origin: ORIGIN_2 })).rejects.toThrow(
      "SESSION_RUNNING",
    );
  ...
  it("standalone auto-compact after origin turn must not inherit origin", async () => {
    ...
    await session.send("go", { origin: SCHED_ORIGIN });
    await emit(
      { type: "text", data: { text: "goal answer", isFinal: true } },
      { keepRunning: true },
    );
    await emit(
      { type: "done", data: { reason: "user-turn-done" } },
      { keepRunning: true },
    );
    await emit(
      { type: "text", data: { text: "standalone compact", isFinal: true } },
      { keepRunning: true },
    );
  ...
  it("在所有 listener 收到事件前脱敏 terminal error 与 failed done 载荷", async () => {
    ...
    for (const events of [first, second]) {
      ...
      const rawError = (
        events[1]!.data as { raw: { error: Record<string, string> } }
      ).raw.error;
  ...
  it("preserves a non-secret auth status and redacts nested Codex error details", async () => {
    ...
    await emit({
      type: "error",
      data: {
        message: "Authorization: Bearer secret-token, status=401",
        isTerminal: true,
      },
    });
    await emit({
    ...
    expect(
      (seen[0]!.data as { message: string; errorStatus: number }).message,
    ...
    expect(
      (
        seen[1]!.data as {
          raw: { error: { codexErrorInfo: { message: string } } };
        }
      ).raw.error.codexErrorInfo.message,
  ...
  it("preserves a rate-limit status after redacting the error message", async () => {
    ...
    await emit({
      type: "error",
      data: {
        message: "Authorization: Bearer secret-token, status=429",
        isTerminal: true,
      },
    });
  ...
  it("does not derive a status from a credential fragment", async () => {
    ...
    await emit({
      type: "error",
      data: {
        message: "Authorization: Bearer tok-401-x; upstream 500",
        isTerminal: true,
      },
    });

    expect(
      (seen[0]!.data as { errorStatus?: number }).errorStatus,
    ).toBeUndefined();
    expect((seen[0]!.data as { message: string }).message).toBe(
  ...
  it("preserves a quota marker after redacting the error message", async () => {
    ...
    await emit({
      type: "error",
      data: {
        message: "Authorization: Bearer secret-token, quota exhausted",
        isTerminal: true,
      },
    });
  ...
  it("redacts failed task summaries before listener fan-out", async () => {
    ...
    expect((seen[0]!.data as { summary: string }).summary).toBe(
      "task failed: password=[REDACTED]",
    );
