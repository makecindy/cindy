...
describe("review read scope", () => {
  ...
  it("resolves symlinks before checking both scope and credential policy", async () => {
    ...
    const outsideLinkDir = path.join(workspace, "outside-link");
    const keyDir = path.join(root, ".ssh");
    const key = path.join(keyDir, "id_ed25519");
    const keyLinkDir = path.join(workspace, "key-link");
    await fs.mkdir(workspace);
    await fs.mkdir(outsideDir);
    await fs.mkdir(keyDir);
    await fs.writeFile(outside, "outside");
    await fs.writeFile(key, "private-key");
    await fs.symlink(outsideDir, outsideLinkDir, "junction");
    await fs.symlink(keyDir, keyLinkDir, "junction");

    const outsideLink = path.join(outsideLinkDir, "outside.txt");
    const keyLink = path.join(keyLinkDir, "id_ed25519");
