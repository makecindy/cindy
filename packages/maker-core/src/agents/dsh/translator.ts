...
export function translateDshEvent(event: DshSessionEvent, queue: AsyncQueue<AgentEvent>, ctx: DshTranslateContext): void {
  ...
  if (event.type === 'tool/call' && data) { const id = typeof data.callId === 'string' ? data.callId : `dsh-tool-${Date.now()}`; queue.push({ type: 'tool_use', data: { toolUseId: id, name: String(data.name ?? 'tool'), input: parseArguments(data.arguments) }, source: 'dsh' }); return; }
  if (event.type === 'tool/result' && data) { const id = typeof data.callId === 'string' ? data.callId : 'dsh-tool-result'; const fullText = typeof data.message === 'string' ? data.message : JSON.stringify(data.message ?? ''); queue.push({ type: 'tool_result_full', data: { toolUseId: id, fullText, isError: Boolean(data.error) }, source: 'dsh' }); return; }
...
export function settleDshTurnOnIdle(queue: AsyncQueue<AgentEvent>, ctx: DshTranslateContext): void {
  ...
  queue.push({ type: 'done', data: { result: ctx.finalText, reason: ctx.pendingTurnEndReason, usage: ctx.usage }, source: 'dsh' });
