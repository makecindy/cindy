...
export class DshAgent extends BaseAgent {
  ...
  private static baseCapabilities(): Capabilities {
    ...
    return {
      switchModel: unavailable, availableModels: [{ id: 'deepseek-v4-flash', displayName: 'DeepSeek V4 Flash', contextWindow: 1_000_000, efforts: [], defaultEffort: null }], hasFastMode: false, effort: unavailable, effortLevels: [], reasoningDisplay: ['full'], permissionModes: [], setPermissionModeMidSession: unavailable,
  ...
  override async startSession(opts: StartSessionOptions): Promise<AgentSessionHandle> {
    ...
    try {
      ...
      if (opts.remoteHostId) {
        ...
        transport = createDshStdioTransport({ nodePath: vendor.dshNodePath ?? process.execPath, binPath: vendor.dshBinPath ?? this.deps.binaryPath, configPath, cwd: opts.workingDir, env: { ...process.env, DEEPSEEK_API_KEY: apiKey, DSH_CWD: opts.workingDir, DSH_SESSION_ROOT: sessionRoot }, logger: this.deps.logger });
      }
      ...
      return {
        get id() { return sessionId; }, agentKind: 'dsh', model: opts.model || 'deepseek-v4-flash', send,
    ...
    } catch (error) { await cleanTemp(); throw error; }
  }
}
