  ...
  async startSession(opts: StartSessionOptions): Promise<AgentSessionHandle> {
        ...
        (async (): Promise<void> => {
          for await (const msg of inputQueue) {
            ...
            try {
              ...
              if (pending) {
                // Timeout cannot cancel a backpressured write. Handle response
                // failure promptly so the existing remote close path can run,
                // but retain the lease until transport or a correlated response
                // establishes the submission boundary.
                confirmedUndispatched = false;
                const submissionBoundary = Promise.race([
                  pending.submitted,
                  pending.response.then(
                    () => undefined,
                    (error) => remoteTransport.isAuthoritativeResponseError?.(error) === true
                      ? undefined
                      : pending.submitted,
                  ),
                ]);
                ...
                void releaseAfterSubmission.catch((error: unknown) => {
                  log.warn('cc remote: dispatch submission settlement failed', {
                    error: error instanceof Error ? error.message : String(error),
                  });
                });
                try {
    ...
    async function recoverInvalidResume(
      ...
      try {
        ...
        releaseGate();
        if (replayInput) {
          const replayGeneration = turnState.generation;
          ...
          if (
            closed ||
            !turnInFlight ||
            turnState.generation !== replayGeneration ||
            turnState.interruptRequested
          ) {
    ...
    const handle: AgentSessionHandle = {
      ...
      async send(message: UserMessage, sendOpts?: SendOptions) {
        ...
        try {
          ...
          try {
            // Stop may win while the durable writer lease is being acquired.
            // This input has not reached the queue, so settle it as undispatched.
            if (sendOpts?.signal?.aborted) {
              throw new Error('Claude send cancelled before acceptance');
            }
