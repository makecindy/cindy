...
import {
  detectGrokBuildOnPath,
  grokAcpInitializeLooksAuthenticated,
  probeGrokBuildAcp,
...
describe('grok-build detection', () => {
  ...
  it('treats sign-in-only authMethods as logged-out', async () => {
  ...
  it('treats grok cached_token default as ready even when grok.com is also listed', async () => {
    const spawnImpl = fakeSpawn((_stdin, stdout) => {
      stdout.write(`${JSON.stringify({
        jsonrpc: '2.0',
        id: 1,
    ...
    expect(result.identity).toBe('grok');
  });

  it('recognizes cached_token without reading auth.json', () => {
    expect(grokAcpInitializeLooksAuthenticated({ authMethods: [] })).toBe(true);
    expect(
      grokAcpInitializeLooksAuthenticated({
