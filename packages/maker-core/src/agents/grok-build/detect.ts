 ...
 * initialize: empty authMethods, or grok's cached_token default, means logged in.
 * Live grok always lists authMethods (cached_token + grok.com) even when already
 * signed in; treating any non-empty list as logged-out is wrong.
 *
...
/** Live grok lists sign-in methods even when a cached token is already the default. */
export function grokAcpInitializeLooksAuthenticated(init: {
  authMethods?: ReadonlyArray<{ id: string }>;
  _meta?: { defaultAuthMethodId?: string };
}): boolean {
  const methods = init.authMethods ?? [];
  if (methods.length === 0) return true;
  if (init._meta?.defaultAuthMethodId === 'cached_token') return true;
  return methods.some((method) => method.id === 'cached_token');
}
...
export async function probeGrokBuildAcp(options: ProbeGrokBuildOptions): Promise<GrokBuildProbeResult> {
  ...
  try {
    ...
    if (!grokAcpInitializeLooksAuthenticated(init)) {
      return {
        status: 'logged-out',
        binaryPath: options.binaryPath,
        agentVersion: init.agentInfo?.version,
