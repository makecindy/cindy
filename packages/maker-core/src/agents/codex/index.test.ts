...
describe('CodexAgent MCP thread context hooks', () => {
  ...
  it('invalidates the Codex reviewer cache when the resolved route revision changes', async () => {
    ...
    const host = installFakeHost(agent, (method) => (
      method === Method.TurnStart ? { turn: { id: 'turn-route-cache' } } : undefined
    ));
    const handle = await agent.startSession({
      sessionId: 'codex-route-revision-cache', model: 'gpt-5.5', providerId: 'xd',
      workingDir: '/repo', permissionMode: 'auto',
    });
    await handle.send({ type: 'user', content: 'Check this project for type errors' });
    const handlers = host.getThreadHandlers();
    if (!handlers?.commandExecutionApproval) throw new Error('expected command approval');
    const command = 'pnpm --filter desktop run typecheck';
    await expect(handlers.commandExecutionApproval({
      threadId: 'start-thread-id', turnId: 'turn-route-cache', itemId: 'route-cache-a',
      command, cwd: '/repo',
    })).resolves.toEqual({ decision: 'accept' });
    routeRevision = 'sha256:route-b';
    await expect(handlers.commandExecutionApproval({
      threadId: 'start-thread-id', turnId: 'turn-route-cache', itemId: 'route-cache-b',
      command, cwd: '/repo',
    })).resolves.toEqual({ decision: 'decline' });
    expect(reviewer).toHaveBeenCalledTimes(2);
    await handle.close();
