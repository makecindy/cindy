...
export class AppServerHost {
  ...
  async request<R = unknown>(
    ...
    opts?: {
      ...
      /** Reentrant synchronous check after lease acquisition and immediately before the queued write. */
  ...
  private async requestAfterStarted<R>(
    ...
    acquireSubmissionLease:
      | (() =>
          ...
          | Promise<VendorDispatchLeaseRelease>)
      | undefined,
    beforeSubmission: (() => void) | undefined,
    deferCorrelatedRejectionSettlement: boolean,
  ): Promise<R> {
    ...
    try {
      ...
      pending = client.requestWithSubmission<R>(
        method,
        params,
        timeoutMs === undefined && beforeSubmission === undefined
          ? undefined
          : { timeoutMs, beforeSubmission },
      );
    } catch (error) {
