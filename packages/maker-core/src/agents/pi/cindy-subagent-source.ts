...
export const CINDY_SUBAGENT_ENV = {
  ...
  /** Cindy-generated CommonJS runner loaded by a real Node-compatible executable. */
...
async function waitForDurableRun(launched, signal, ctx, onStatus) {
  ...
  let stopDeadline = 0;
  let runnerExit = null;
  launched.exited.then(function (exit) { runnerExit = exit; });
  for (;;) {
    let status;
    ...
    if (runnerExit) {
      return launched.failureStatus(
        'Durable runner exited before publishing a terminal status'
          + (runnerExit.error ? ': ' + runnerExit.error : '')
          + (runnerExit.signal ? ' (signal ' + runnerExit.signal + ')' : '')
          + (typeof runnerExit.code === 'number' ? ' (exit ' + runnerExit.code + ')' : ''),
      );
    }
...
function launchDurableRun(binary, tasks, runtime, taskId, mode, context, displayTitle, interactiveOwner, timeoutMs) {
  ...
  const nodeExecutable = process.env[NODE_EXECUTABLE_ENV];
  ...
  const runner = spawn(nodeExecutable, [durableRunnerFile, configPath], {
    cwd: process.cwd(),
  ...
  const persistFailureIfNeeded = function (message) {
    try {
      ...
      /* No safe fallback remains when both process launch and private status persistence fail. */
    }
  };
  let settleExit;
  const exited = new Promise(function (resolve) { settleExit = resolve; });
  let exitSettled = false;
  const settleRunnerExit = function (exit) {
    if (exitSettled) return;
    exitSettled = true;
    settleExit(exit);
  };
  runner.once('error', function (error) {
    const message = 'Durable runner failed to start: ' + String(error);
    persistFailureIfNeeded(message);
    settleRunnerExit({ code: null, signal: null, error: String(error) });
  });
  runner.once('close', function (code, signalName) {
    const message = 'Durable runner exited before publishing a terminal status'
...
export default async function cindySubagent(pi: any) {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId: string, params: unknown, signal: AbortSignal, onUpdate: any, ctx: any) {
      ...
      report('running');
      const launched = launchDurableRun(
        binary,
        tasks,
        runtime,
        taskId,
