  ...
  private async startSessionWhileRunning(opts: StartSessionOptions): Promise<AgentSessionHandle> {
    ...
    try {
      ...
      const spawnEnv: NodeJS.ProcessEnv = {
        ...
        ...nativeEnv,
        CINDY_PI_NATIVE_PROVIDER_ADAPTERS: JSON.stringify(nativeProviders.filter(provider => provider.adapterProvider && provider.apiKeyEnvVar).map(provider => ({ id: provider.id, name: provider.name, provider: provider.adapterProvider, keyEnv: provider.apiKeyEnvVar }))),
