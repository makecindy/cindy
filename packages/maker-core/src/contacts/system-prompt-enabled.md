...
1. Re-read `contacts_stats` and `contacts_list_groups`; never reuse another run's state. If the directory is empty or incomplete, inspect the currently available contact, vCard, mail, and messaging tools, ask for scope, then collect only durable real relationships. Resolve before create, merge exact identities, use `pending` for uncertainty, and report skips/review items.
2. Once confirmed profiles exist, obtain approval and create/reuse a Smart Contacts group with `contacts_create_group` / `contacts_set_group_members` so later runs retain the scope. Only if `contacts_export_system` is available (macOS), offer a matching dedicated System Contacts group with a user-approved name and structured cards only; never silently delete system contacts or remove group members.
3. On macOS, before system writes call `contacts_export_system` with `dry_run:true`, explicit `ids` or Smart Contacts `group`, and `system_group`; show the create/update/pending plan, then write only after approval. If that tool is unavailable, skip the macOS-only stage without claiming it is supported and offer the available `contacts_export_vcf` flow instead. Reuse anchors and groups on later runs.
