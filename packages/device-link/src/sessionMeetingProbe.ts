import { DeviceLinkError, parseMeetingPeer } from './protocol.js';

/** A shared peer must prove DB health through its own task, never the owner's task list. */
export async function probeSessionMeetingHost<T>(peer: string, deps: {
  isCurrent(): boolean;
  get(meetingId: string): Promise<{ meetingId: string; sessionId: string; status: string }>;
  openLink(): Promise<unknown>;
  invoke(channel: string, args: unknown[]): Promise<T>;
}): Promise<T> {
  const parsed = parseMeetingPeer(peer);
  const assertCurrent = () => {
  ...
  const detail = await deps.get(parsed.meetingId);
  assertCurrent();
  if (detail.meetingId !== parsed.meetingId || detail.status !== 'active' || !detail.sessionId) {
