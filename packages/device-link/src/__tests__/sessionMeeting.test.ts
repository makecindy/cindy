...
import {
  authorizeSessionMeetingOperation as authorize,
  parseSessionMeetingSnapshot as parse,
  isSessionMeetingAttachment, sessionMeetingTopics,
} from '../sessionMeeting.js';
import { buildAttachmentOssRef } from '../attachmentOssRef.js';

const snapshot = () => ({
  meetingId: 'meeting-1', sessionId: 'session-1', ownerAccountId: 'owner', hostDeviceId: 'desktop',
...
describe('meeting authorization', () => {
  it('keeps task subscriptions but removes the full-device list from shared peers', () => {
    expect(sessionMeetingTopics('meeting~shared~host', ['sessions', 'session:task'])).toEqual(['session:task']);
    expect(sessionMeetingTopics('my-desktop', ['sessions', 'session:task'])).toEqual(['sessions', 'session:task']);
  });
  it('only permits attachment references in this sharing namespace', () => {
    ...
    expect(isSessionMeetingAttachment(ref('cindy/shared-task/shared/u/file.png'), 'shared')).toBe(true);
    for (const key of ['cindy/shared-task/other/u/file.png', 'cindy/device-link/u/file.png', 'cindy/shared-task/shared/u/../file.png']) {
      expect(isSessionMeetingAttachment(ref(key), 'shared')).toBe(false);
    }
    expect(isSessionMeetingAttachment('file:///private/file.png', 'shared')).toBe(false);
  });
  ...
  it.each(['approval.resolve', 'permission.configure', 'workdir.configure', 'plugins.configure',
    'history.delete', 'session.archive', 'session.export', 'session.fork', 'background.create', 'schedule.create', 'meeting.manage'])(
  ...
  it('closed and not-yet-loaded meetings cannot authorize their owner or guests', () => {
...
describe('meeting authority decoding', () => {
