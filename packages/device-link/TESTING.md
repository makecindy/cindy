...
| 协议/客户端包 | `packages/device-link/src/{client,allowlist,topics,protocol,transport}.ts` | 纯单测 + 假 `WsLike`/timing 注入(`__tests__/client.test.ts` 等) | 指数退避重连 + epoch 丢弃过期 socket 回调 / 心跳 pong-miss 强制重连 / 请求按 id 配对 + 超时 / 断线立即失败旧协议请求、保留可靠 pending / 可靠传输按序交付、handler 接收后 ACK、skip/baseSeq 收敛、分片/背压/有界重传、队头容量优先 / 离线时 send 静默忽略 / 版本不匹配保持离线 / allowlist 默认拒绝 + 危险模式不变式 / topicForPush 路由 |
| 桌面 main | `apps/desktop/src/main/device-link/*` + `localDb/ipc/sessions.ts` | 假 client + 内存 registry + mock drizzle/electron(`__tests__/deviceLink*.test.ts`) | invoke 双层门禁(remoteControlEnabled + allowlist)+ CHANNEL_NOT_ALLOWED / requestId 在途+完成去重 / `invoke-result` 发送背压进入有界 outbox、presence 瞬断后补发 / 永久挂起 IPC 到期释放执行槽但不声称取消底层副作用 / 不可序列化结果明确失败 / 订阅控制帧用 server 填的 src 作 controllerDeviceId(防伪造)/ topic fan-out 只发订阅者 / set-* 回流 persistSessionFields + 广播 / presence-offline 清僵尸订阅 / remote-workdir-guard 失败 fail-safe / 错误经隧道 `[CODE]` 透传 |
