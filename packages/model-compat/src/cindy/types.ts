  ...
  const invalidRequest =
    (typeof code === 'string' &&
      (code.includes('invalid_request') ||
        code.includes('unsupported_media') ||
        code.includes('unprocessable'))) ||
    (typeof type === 'string' &&
      (type.toLowerCase().includes('invalid_request') ||
        type.toLowerCase().includes('unsupported_media') ||
        type.toLowerCase().includes('unprocessable')));
  if (invalidRequest && !message.startsWith(UNSUPPORTED_RESPONSES_FEATURE_MESSAGE_PREFIX)) {
  ...
  if (code === 'upstream_error' && typeof message === 'string') {
    ...
    if (inner && typeof inner === 'object' && inner !== null) {
      ...
      if (typeof innerErr === 'string') {
        ...
        const innerInvalid =
          (typeof ic === 'string' &&
            (ic.includes('invalid_request') ||
              ic.includes('unsupported_media') ||
              ic.includes('unprocessable'))) ||
          (typeof it === 'string' &&
            (it.toLowerCase().includes('invalid_request') ||
