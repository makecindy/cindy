...
/** Parse the machine-readable code prefix from a host LocalPreviewError. */
...
export function registerBrowserTools(registry: BrowserToolRegistry, deps: BrowserMcpDeps): void {
  ...
  registry.register({
    ...
    description:
      ...
      '本地 HTML 文件请使用 previewLocalHtml(沙箱化预览),不要用 file:// 或裸跑浏览器。',
    ...
    handler: async (args) => {
      ...
      try {
        ...
        // previewLocalHtml: sandboxed local HTML preview (MCP-only action,
        // not forwarded to the runtime). Issues a tokenized loopback URL via
        // the host, then opens it through the ordinary open/navigate path.
