...
export const TEAMMATE_CONTROL_GUIDANCE = [
  ...
  'When the user specifies an existing Session, discover send_to_existing_session in the helper bots category. This separate entry confirms the exact target and message through the Host and preserves that Session; use the same idempotency key for retries. It does not create a replacement or change the target model, permissions or pause state. A queued receipt is not consumption or completion. If the entry is unavailable, report the capability gap; do not substitute a fresh background execution.',
