 ...
 *    自动附加的 Harness / Model ID 是当前任务的;OS 来自提交客户端本机。agent 按反馈本身判断是否在正文补充用户说明的实际故障环境,不要把自动附加的当成出问题的那个,也不要复制快照。
...
const DESCRIPTION = [
  ...
  '6) 系统自动附加「提交时的任务环境」快照(客户端版本 / 版本区域 / 提交客户端 OS / 当前任务的 Harness / 模型 ID / 界面语言);GitHub 作者就是确认卡片显示的身份——这些快照字段无需也无法由你填写。系统自动附加的 Harness / Model ID 是当前任务的,不要复制进正文,也不要当成出问题的那个。用户说明的实际故障 Harness / 模型 / 推理强度,只在真正有助于定位时写入正文。版本区域是用户装的哪个区域构建(CN / Dev,默认的 global 构建不标注),构建期烘焙,你猜不到也不要写。',
