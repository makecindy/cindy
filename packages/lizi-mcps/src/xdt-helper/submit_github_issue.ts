 ...
 *  - 环境信息由 host 自动附加为「提交时的任务环境」,agent 无法也无需填写。
 *    自动附加的 Harness / Model ID 是当前任务的。agent 按反馈本身判断是否在正文补充运行信息,不要把自动附加的当成出问题的那个。
...
const DESCRIPTION = [
  ...
  '【流程硬约束】',
  '1) 调用前先与用户对话,把这条反馈整理到足以让维护者看懂、能行动。缺什么问什么,已说过的不要重复,不够清楚不要急着调用。问什么、问多深、正文怎么组织,由你按反馈本身判断,不要套固定问卷或章节清单。普通建议不必按缺陷来问,和环境无关不必追问 Harness / 模型 / 推理强度。用户不知道的标未知,不要猜。系统自动附加的 Harness / Model ID 是当前任务的,不要当成出问题的那个。功能建议写用户能看见的场景和诉求;不要写仓库路径、实现方案、验收清单或内部架构推测,除非用户明确要求。',
  ...
  '6) 环境信息(客户端版本 / 版本区域 / OS / Harness / 模型 ID / 界面语言)由系统自动附加为「提交时的任务环境」;GitHub 作者就是确认卡片显示的身份——都无需也无法由你填写。自动附加的 Harness / Model ID 是当前任务的,不要写进正文当环境信息,也不要当成出问题的那个。正文只补充对定位真正有用、且用户已说明的运行信息。版本区域是用户装的哪个区域构建(CN / Dev,默认的 global 构建不标注),构建期烘焙,你猜不到也不要写。',
...
const D_BODY =
  ...
  'Harness / 模型 / 推理强度等只在真正有助于定位时写入,不要当固定栏目。' +
  '功能建议不要写仓库路径、实现方案或验收清单,除非用户明确要求。禁止声称截图已附在本 issue——本工具不会上传图片。' +
  '不要写环境信息(客户端版本 / 版本区域 / OS / Harness / 模型 ID / 界面语言)和提交人——系统会自动附加为「提交时的任务环境」。';
