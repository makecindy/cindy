...
const DESCRIPTION =
  ...
  '目标文件已存在时拒绝,不覆盖)。导出的包不加密(需要密码请让用户在 GUI 里导出)。' +
  '体积超限时返回 OVERSIZE,可用 exclude_media=true 只导出文本与转录重试。' +
  '限制:仅本机会话可用(SSH 远程任务里不可用);远程会话、协同 worker、已删除会话不能导出。每次调用需经用户确认。' +
...
export function registerExportSessionTool(
  ...
  registry.register({
    ...
    handler: async ({ session_id, target_path, exclude_media }) => {
      ...
      // 导出落盘在 Desktop 本机;SSH 远程任务的 target_path 语义在远端,这里不代写本机文件。
      if (ctx.remoteHostId) {
        return errorPayload(
          'PRECONDITION_FAILED',
          '当前是 SSH 远程任务,export_session 只在本机任务里可用(导出文件写在 Desktop 本机)。',
        );
      }
