...
export function registerBotDelegationTools(
  ...
  registry.register({
    ...
    description: [
      ...
      '只传必要上下文引用，不复制父任务全部历史。timeout_ms 是持久执行边界。普通研究、代码修改或需要启动 Agent 的工作不要填写 timeout_ms，省略后使用 30 分钟默认值；只有用户明确要求更短或更长的截止时间时才填写。',
    ].join('\n'),
    inputShape: {
  ...
  registry.register({
    name: 'start_cindy_task',
    category: 'bots',
    description: [
      '把一件大活开成一条真正的 Cindy 任务（写代码、跑工程、长时间的重活）。子任务出现在用户的主任务列表里，有完整的 Cindy 能力面与自己的权限确认；完成后结果自动回到当前对话。',
      'working_dir 缺省用你自己的 Home workspace；需要在某个已存在目录里干活时传绝对路径。timeout_ms 省略时默认 30 分钟。',
    ].join('\n'),
    inputShape: {
      objective: z.string().min(1).max(12_000),
    ...
    handler: async ({ objective, title, working_dir, timeout_ms }) => {
      const sessionId = callerSessionId(deps);
      if (!sessionId) return missingSession();
      const result = await deps.callbacks.delegateToCindy({
        callerSessionId: sessionId,
        objective,
        title,
