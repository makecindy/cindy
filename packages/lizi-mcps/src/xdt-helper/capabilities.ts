...
export const CAPABILITIES: readonly CapabilityEntry[] = [
  ...
  },
  {
    ...
    detail: [
      ...
      '侧栏会话菜单里的移动与分叉 agent 同样可代办(都在 cindy_helper control 类,与 GUI 走同一条主进程路径,侧栏即时刷新):move_sessions(移动到项目 / 移回对话)、fork_session(在某条消息处分叉出新会话)。两者都不支持远程会话;move_sessions 还会拦下运行中与被 IM 接管中的会话(fork 只读历史快照,不受这两项限制)。',
