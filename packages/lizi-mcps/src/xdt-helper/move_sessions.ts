...
const DESCRIPTION =
  ...
  'working_dir 必须是真实目录的绝对路径:传软链或别名会以 INVALID_ARGS 拒绝并告知真实路径,' +
  '这样用户批准的、落库的与日后生效的是同一个目录。' +
...
export function registerMoveSessionsTool(
  ...
  registry.register({
    ...
    handler: async ({ session_ids, target_kind, working_dir }) => {
      ...
      return okPayload({
        target_kind,
        // 顶层路径取 host 实际写入的目录,避免与 moved[].working_dir 自相矛盾。
        working_dir: target_kind === 'project' ? (result.moved[0]?.workingDir ?? working_dir) : null,
