...
export function registerTelegramDeliveryTools(registry: SchedulerToolRegistry, deps: SchedulerMcpDeps): void {
  ...
  registry.register({
    ...
    description: '经官方 Telegram IM 直接发送定时结果或用户明确授权的测试；发送遵循 Host 逐次授权策略，不因属于 scheduler 而免审。先查 status，完整复用 target；按分段持久保存唯一 idempotencyKey 和原文/展示哈希。text 是最终 HTML 或纯文本；plain 每段最多 4096 个 UTF-16 单位；HTML 也保守限制原始源码为 4096，标签和实体计入预算，须在调用前按完整标签分段。started/unknown 不得换 key 重发；sent 表示真实消息 ID 与绑定目标匹配；result.sentMessage 包含实际正文/entities，调用者应对照预期展示稿验真。formatVerified=false 表示 Host 未代替调用者比较格式。',
    inputShape: {
      idempotencyKey: z.string().min(1).max(200), target,
      text: z.string().min(1).max(4096), tier: z.enum(['html', 'plain']),
      sourceSha256: z.string().regex(/^[a-f0-9]{64}$/),
      presentationSha256: z.string().regex(/^[a-f0-9]{64}$/),
    },
    handler: async input => {
      // Reject before the Host can persist an idempotency claim. A conservative
      // HTML source bound also bounds rendered text without parsing/truncation.
      if (input.text.length > 4096) {
        return buildJsonResult({ ok: false, code: 'INVALID_DELIVERY_INPUT',
          message: 'Text source exceeds 4096 UTF-16 units; split it before sending.' }, true);
      }
