 ...
 *    - 'convert' : render_pdf / office_to_pdf       —— 出 PDF
 *    - 'read'    : read_sheet                       —— 读已有表格(也是产出自检的入口)
 *
 * 边界:
 *  - 本包不 import electron(既有铁律)。render_pdf 靠 Chromium printToPDF,渲染函数
 *    由 desktop main 经 deps.renderHtmlToPdf 注入;host 没注入就不注册该工具。
 *  - 其余四个生成/读取工具依赖的 docx / pptxgenjs / exceljs / marked 都是纯 JS,
 *    与本包已有的 sharp / googleapis 同级,直接在包内实现,不再绕一层注入。
...
const D_LIST_TOOLS =
  ...
  'PDF / Word / Excel / PPT 这类真文件时先来这里。' +
  '不传 category → 返回所有类目 + 每个类目工具数量。' +
  '传 category=author → 从结构化内容直接生成 Office 文件(make_docx 出 Word、' +
  'make_pptx 出演示文稿、make_xlsx 出 Excel);' +
  '传 category=convert → 出 PDF(render_pdf 把 HTML 用内置 Chromium 排版成 PDF,零系统依赖;' +
  'office_to_pdf 把已有 Office 文件转 PDF,需要用户装了 LibreOffice);' +
  '传 category=read → 读已有表格(read_sheet 支持 xlsx / csv / tsv,也用来回验自己刚生成的文件)。' +
  '获取工具名后用 call_tool({name, args}) 执行。' +
  '所有输入输出路径都必须在当前任务的工作目录内。';

const D_CALL_TOOL =
  '调用 cindy_docs 中的某个具体工具(render_pdf / make_docx / make_pptx / make_xlsx / ' +
  'read_sheet / office_to_pdf)。先用 list_tools 拿工具名 + 完整参数说明。' +
  ...
  '`SOFFICE_NOT_FOUND` = 机器上没装 LibreOffice(office_to_pdf 专有,retryable:false —— ' +
  '照 hint 如实告诉用户怎么装,不要重试)。' +
  '生成类工具成功后返回 path / relativePath / bytes;bytes 明显偏小意味着产物可能是空的,应回验而不是直接交付。';
...
export function createCindyDocsMcpServer(
  ...
  registerOfficeToPdfTool(registry, sessionCtx);
  // render_pdf 只在 host 提供了 Chromium 渲染回调时注册 —— 没有渲染能力的宿主
  // 里让模型看不到它,好过看见了调用再拿到一个「不可用」。
