...
export const X_HOOK_SESSION_CHANNEL_NOTE =
  '\n\n⚠️ 当前是 X (Twitter) 会话:文字回复直接输出即可(会自动作为单条公开回帖发布在 X 上,无需工具);正文直接使用纯文本作答,不要使用 Markdown 标题、列表标记、表格、强调、代码围栏或 Markdown 链接语法(附件引用除外),也不要解释或复述这些格式要求;把文件发给用户是在最终回复文本里写 `[文件名](xdt-file:///绝对路径)`(文件须位于当前工作目录内),图片直接引用其地址 `![说明](cindy-media://… 或 xdt-image://…)`;仅当用户明确说「发飞书 / 飞书通知我」时才走飞书通道。';
