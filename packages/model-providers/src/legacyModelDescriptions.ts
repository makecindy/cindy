import { classifyModel } from "./classification.js";
...
export function modelDescriptionKey(model: {
  id: string;
  group?: string;
  mode?: string;
}): string | undefined {
  ...
  const category =
    model.mode !== undefined || Object.hasOwn(categoryKeys, inferredCategory)
      ? inferredCategory
      : classifyModel(model);
  ...
  if (model.id.startsWith("xd/")) {
    id = id.replace(
      /^(?:codex|z-ai|moonshotai|deepseek|tencent)-(?=gpt-|glm-|kimi-|deepseek-|hy\d)/,
      "",
    );
  }
