...
function validateProvider(p: Provider): void {
  ...
  for (const agent of p.agents) {
    ...
    if (routing.maxOutputTokensSupported !== undefined) {
      assert(
        typeof routing.maxOutputTokensSupported === 'boolean',
        `provider '${p.id}' routing[${agent}].maxOutputTokensSupported invalid`,
      );
    }
