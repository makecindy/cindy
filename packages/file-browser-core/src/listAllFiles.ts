 ...
 *
 * 这里**不接**「显示被忽略的目录」开关(showIgnoredDirs):该开关管的是文件树
 * 能列出哪些目录,而这份清单服务的是文件名筛选。开关打开后树里看得到的 build /
 * dist 仍然不在清单里——文件名筛选与内容搜索的过滤语义一直存在这种差异(rg 那
 * 一路本来就只吃 .gitignore),不从这条 fallback 路径单独改变它。
 *
