...
const D_MEDIA = [
  ...
  "list_models 合并当前账号的 Model Access 模型与 Host 已连接的本地订阅媒体来源。Gateway 模型按 modalities、Guide operation 与客户端协议筛选；本地视频按实际注册的 Provider 能力、授权、启用及显示状态筛选，缺项不等于账号没有订阅权益。",
  ...
  "本地订阅视频同样走 prepare → request → poll；prepare 返回该 Provider 的真实 input_schema，OAuth 由 Host 复用。已有句柄只查询原任务；视频已生成但下载或入库失败时继续 poll 原 invocation_id，不新建生成单。当前没有视频取消操作或百分比进度承诺。",
  "本地订阅图生视频的 image 可传受管图片地址，或规范 PNG/JPEG Base64 data URL（每张最多 20 MiB、16777216 像素、最长边 8192，须完整单帧）；按 prepare 说明使用。不接受文件路径、HTTP 或 SVG。不要在说明、日志或错误中回显图像 body。",
  "已生成但交付失败时，result_retained 表示原结果保留；delivery_error 仅含阶段、hostname、HTTP 状态或安全错误码。retryable=true 时仅 poll 原 invocation_id；不可据下载失败重新 prepare/request。地址疑似失效时 Host 只刷新原 task，不重新生成。",
